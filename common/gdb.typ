...
- New alternative: #emph[lldb] (#link("https://lldb.llvm.org/")) 
  from the LLVM project.

#colbreak() #align(center, [#image("gdb.png", width: 90%)]) 
===  GDB crash course (1/3)

- GDB is used mainly to debug a process by starting it with #emph[gdb]
...
- On the target, run a program through `gdbserver`. 
  ...
  `gdbserver :<port> <executable> <args>` `gdbserver /dev/ttyS0
  <executable> <args>`
