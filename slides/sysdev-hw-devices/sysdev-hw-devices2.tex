...
=== {Accessing devices directly from user-space}
  ...
  \item The kernel provides the following mechanisms, depending on the
    ...
    \item USB: `/dev/bus/usb`, through \href{https://libusb.info/}{libusb}
...
=== {Finding Linux kernel drivers}
  \begin{itemize}
  \item `grep` in the Linux kernel tree is your {\em best friend}
    \begin{itemize}
...
=== [fragile]{Finding Linux kernel drivers: an example}
  \begin{itemize}
  \item You have a
...
=== {Devices: everything is a file}
  ...
  \item It allows applications to manipulate all {\em system objects} with
    the normal file API (`open`, `read`, `write`,
    `close`, etc.)
...
=== [fragile]{Device files examples}

Example of device files in a Linux system

\small
\begin{verbatim}
$ ls -l /dev/ttyS0 /dev/tty1 /dev/sda /dev/sda1 /dev/sda2 /dev/sdc1 /dev/zero brw-rw---- 1 root disk    8,  0 2011-05-27 08:56 /dev/sda brw-rw---- 1 root disk    8,  1 2011-05-27 08:56 /dev/sda1
...
=== {Creating device files}
  \begin{itemize}
    ...
    `mknod` command
    \begin{itemize}
    \item `mknod /dev/<device> [c|b] major minor`
    ...
    \end{itemize}
  \item The `devtmpfs` virtual filesystem can be mounted on
    `/dev` $\rightarrow$ the kernel automatically creates/removes
    device files
    \begin{itemize}
...
=== {Better handling of device files: {\em udev} and {\em mdev}}
  \begin{itemize}
  ...
  \item {\bf udev}
    ...
    \item can create/remove device files (but that's done by
      ...
      devices
    \item according to rules files in `/lib/udev/rules.d` and
      `/etc/udev/rules.d`
...
=== {Examples of user-space interfaces in {\tt /dev}}
  \begin{itemize}
  \item Serial-ports: `/dev/ttyS*`, `/dev/ttyUSB*`,
    `/dev/ttyACM*`, etc.
  \item GPIO controllers (modern interface): `/dev/gpiochipX`
  \item Block storage devices: `/dev/sd*`, `/dev/mmcblk*`, `/dev/nvme*`
  \item Flash storage devices: `/dev/mtd*`
  \item Display controllers and GPUs: `/dev/dri/*`
  \item Audio devices: `/dev/snd/*`
  \item Camera devices: `/dev/video*`
  \item Watchdog devices: `/dev/watchdog*`
  \item Input devices: `/dev/input/*`
  \item and many more...
  \end{itemize}
...
=== {{\em sysfs} filesystem}
  \begin{itemize}
  \item `block/`, symlinks to all block devices, in
    `/sys/devices`
  \item `bus/`, one sub-folder by type of bus
  \item `class/`, one sub-folder per class (category of devices):
    input, leds, pwm, etc.
  \item `dev/`
    \begin{itemize}
    \item `block/`, one symlink per block device, named after
      major/minor
    \item `char/`, one symlink per character device, named after
      major/minor
    \end{itemize}
  \item `devices/`, all devices in the system, organized in a
    slightly chaotic way, see \href{https://lwn.net/Articles/646617/}{this article}
  \item `firmware/`, representation of firmware data
    \begin{itemize}
    \item `devicetree/`, directory and file representation of
      Device Tree nodes and properties
    \end{itemize}
  \item `fs/`, properties related to filesystem drivers
  \item `kernel/`, properties related to various kernel subsystems
  \item `module/`, properties about kernel modules
  \item `power/`, power-management related properties
  \end{itemize}


=== [fragile]{{\em sysfs} filesystem example}
  \begin{itemize}
  \item `/sys/bus/i2c/drivers`: all device drivers for devices
...
\end{verbatim}
  ...
  \item `/sys/bus/i2c/devices`: all devices in the system
...
\end{verbatim}
  ...
  \begin{itemize}
  \item `driver`, symlink to the driver directory in `/sys/bus/i2c/drivers`
  \item `of_node`, symlink to the directory for the Device Tree
    node describing this device
  \end{itemize}


=== {Example of driver interfaces in {\em sysfs}}
  \begin{itemize}
  \item All devices are visible in {\em sysfs}, whether they have an
    interface in `/dev` or not
    \begin{itemize}
    \item Usually `/dev` is to access the device
    \item `/sys` is more about properties of the devices
    \end{itemize}
  \item However, some devices only have a {\em sysfs} interface
    \begin{itemize}
    \item LED: `/sys/class/leds`, see \href{https://docs.kernel.org/leds/leds-class.html}{documentation}
    \item PWM: `/sys/class/pwm`, see \href{https://docs.kernel.org/driver-api/pwm.html\#using-pwms-with-the-sysfs-interface}{documentation}
    \item IIO: `/sys/bus/iio`, see \href{https://docs.kernel.org/driver-api/iio/index.html}{documentation}
    \item etc.
    \end{itemize}
  \end{itemize}
...
=== {Accessing GPIOs}
  ...
  \item A new interface recommended: \href{https://git.kernel.org/pub/scm/libs/libgpiod/libgpiod.git/}{libgpiod}
    \begin{itemize}
	\item Based on `/dev/gpiochipx` character devices
...
=== {Other virtual filesystems}
  ...
  \item {\em debugfs}
    \begin{itemize}
    \item Conventionally mounted in `/sys/kernel/debug`
    \item Contains lots of debug information from the kernel,
      including device related
    \item `/sys/kernel/debug/pinctrl` for pin-mux debugging,
      `/sys/kernel/debug/gpio` for GPIO debugging,
      `/sys/kernel/debug/pwm` for PWM debugging, etc.
    \item \url{https://www.kernel.org/doc/html/latest/filesystems/debugfs.html}
    \end{itemize}
  \item {\em configfs}
    \begin{itemize}
    \item Conventionally mounted in `/sys/kernel/config`
...
=== {Module installation and metadata}
  ...
  \item Compiled kernel modules are stored in `.ko` ({\em Kernel Object}) files
  \item Metadata files:
    \begin{itemize}
    \item `modules.dep`
    \item `modules.alias`
    \item `modules.symbols`
    \item `modules.builtin`
    \end{itemize}
  \item Each file has a corresponding `.bin` version, which is an optimized version of the corresponding text file
  \end{itemize}


=== {Module dependencies: {\em modules.dep}}
  \begin{itemize}
  \item Some kernel modules can depend on other modules, based on the
    symbols (functions and data structures) that they use.
  \item Example: the `ubifs` module depends on the `ubi` and
    `mtd` modules.
    \begin{itemize}
    \item `mtd` and `ubi` need to be loaded before `ubifs`
    \end{itemize}
...
=== [fragile]{Module utilities: {\em modinfo}}
  \begin{itemize}
  \item `modinfo <module_name>`, for modules in `/lib/modules`
  \item `modinfo /path/to/module.ko`
  \end{itemize}
...
=== [fragile]{Module utilities: {\em lsmod}}
  ...
  \item Includes
    ...
    \end{itemize}
  \item Information retrieved through `/proc/modules`
  \end{itemize}

  \begin{block}{}
    {\footnotesize
\begin{verbatim}
$ lsmod Module                  Size  Used by tun                    61440  2
tls                   118784  0
rfcomm                 90112  4
...
=== [fragile]{Module utilities: {\em insmod} and {\em rmmod}}
  \begin{itemize}
  \item Basic tools to:
    \begin{itemize}
    \item {\em load} a module: `insmod`
    \item {\em unload} a module: `rmmod`
    \end{itemize}
  \item Basic because:
    \begin{itemize}
    \item Need a full path to the module `.ko` file
  ...
  \begin{block}{}
    {\footnotesize
\begin{verbatim} # insmod /lib/modules/`uname -r`/kernel/fs/fuse/cuse.ko.xz
# rmmod cuse
\end{verbatim}
    }
  \end{block}
...
=== [fragile]{Module utilities: {\em modprobe}}
  \begin{itemize}
  \item {\em modprobe} is the more advanced tool for loading/unloading
    modules
  \item Takes just a module name as argument: `modprobe <module-name>`
  \item Takes care of dependencies automatically, using the
    `modules.dep` file
  \item Supports removing modules using `modprobe -r`, including
...
=== {Passing parameters to modules}
  ...
  \item Through `insmod` or `modprobe`:\\
    `insmod ./usb-storage.ko delay_use=0`\\
    `modprobe usb-storage delay_use=0`
  \item `modprobe` supports configuration files: `/etc/modprobe.conf` or in any file in `/etc/modprobe.d/`:\\
  ...
  \item Through the kernel command line, when the module is built statically into the kernel:\\
    ...
    \begin{itemize}
    \item `usb-storage` is the {\em module name}
    \item `delay_use` is the {\em module parameter name}. It
      specifies a delay before accessing a USB storage device (useful for
      rotating devices).
    \item `0` is the {\em module parameter value}
    \end{itemize}
  \end{itemize}
...
=== {Modules in {\em sysfs}}
  \begin{itemize}
  \item All modules are visible in {\em sysfs}, under `/sys/module/<name>`
  \item Lots of information available about each module
  \item For example, the `/sys/module/<name>/parameters`
...
[
    ...
    \item A tree data structure describing the hardware is written by a
      developer in a {\bf Device Tree Source} file, `.dts`
    \item Processed by the {\bf Device Tree Compiler}, `dtc`
    \item Produces a more efficient representation: {\bf Device Tree
        Blob}, `.dtb`
    \item Additional C preprocessor pass
    \item `.dtb` $\rightarrow$ accurately describes the hardware platform in an {\bf OS-agnostic} way.
    \item `.dtb` $\approx$ few dozens of kilobytes
    \item DTB also called {\bf FDT}, {\em Flattened Device Tree}, once
      loaded into memory.
      \begin{itemize}
      \item `fdt` command in U-Boot
      \item `fdt_` APIs
      \end{itemize}
    \end{itemize}
    \column{0.3\textwidth}
...
[
    ...
    \item Can be {\bf passed} to the operating system by the bootloader
      ...
      \item U-Boot: `boot[z,i,m] <kernel-addr> - <dtb-addr>`
      \item The bootloader can adjust the DTB before passing it to the
        kernel
      \end{itemize}
    \item The DTB parsing can be done using `libfdt`, or ad-hoc
      code
    \end{itemize}
    \column{0.4\textwidth}
...
=== {Where are Device Tree Sources located?}
  ...
  \item In practice, the Linux kernel sources can be considered as the
    ...
    \begin{itemize}
    \item `arch/<ARCH>/boot/dts/<vendor>/`
    \item `arch/arm/boot/dts` (on ARM 32 architecture before Linux 6.5)
    \item $\approx$ 4500 Device Tree Source files (`.dts` and
          `.dtsi`) in Linux as of 6.0.
    \end{itemize}
...
[
    ...
    \item `dtc` only does syntax checking, no semantic validation
    \end{itemize}
    \column{0.5\textwidth}
    \begin{center}
...
=== [fragile]{DT overall structure: simplified example}
...
=== [fragile]{Inheritance and labels}
...
=== [fragile]{Device Tree binding: old style}
  \begin{center}
...
=== [fragile]{Device Tree binding: YAML style}
...
=== [fragile]{Device Tree binding: YAML style example}
    \begin{block}{}
      {\fontsize{5}{6}\selectfont
\begin{minted}{yaml}
examples:
...
=== {Validating Device Tree in Linux}
  \begin{itemize}
  \item `dtc` only does syntactic validation
  \item YAML bindings allow to do semantic validation
  \item Linux kernel `make` rules:
    \begin{itemize}
    \item `make dt_binding_check`\\
      verify that YAML bindings are valid
    \item `make dtbs_check`\\
      validate DTs currently enabled against YAML bindings
    \item `make DT_SCHEMA_FILES=Documentation/devicetree/bindings/trivial-devices.yaml dtbs_check`\\
      validate DTs against a specific YAML binding
    \end{itemize}
  \end{itemize}


=== {The {\tt compatible} property}
  \begin{itemize}
  ...
  \item Examples:
    \begin{itemize}
    \item `compatible = "arm,armv7-timer";`
    \item `compatible = "st,stm32mp1-dwmac", "snps,dwmac-4.20a";`
    \item `compatible = "regulator-fixed";`
    \item `compatible = "gpio-keys";`
    \end{itemize}
  \item Special value: `simple-bus` $\rightarrow$ bus where all
    sub-nodes are memory-mapped devices
  \end{itemize}


=== {{\tt compatible} property and Linux kernel drivers}
...
[
    ...
    \item Linux identifies as {\bf platform devices}:
      \begin{itemize}
      \item Top-level DT nodes with a `compatible` string
      \item Sub-nodes of `simple-bus`
        \begin{itemize}
...
=== [fragile]{Matching with drivers in Linux: platform driver}
...
=== [fragile]{Matching with drivers in Linux: I2C driver}
...
=== [fragile]{{\tt reg} property}
  \begin{itemize}
  \item Most important property after `compatible`
...
\end{onlyenv}
\pause
\item The unit address must be the address of the first `reg`
  entry.
\begin{onlyenv}<4>
\begin{block}{}
\begin{verbatim}
...
=== {Status property}
  \begin{itemize}
  \item The `status` property indicates if the device is really in
    use or not
    \begin{itemize}
    \item `okay` or `ok` $\rightarrow$ the device is really
      in use
    \item any other value, by convention `disabled` $\rightarrow$
      the device is not in use
    \end{itemize}
  \item In Linux, controls if a device is instantiated
  \item In `.dtsi` files describing SoCs: all devices that
    interface to the outside world have `status = "disabled";`
  \item Enabled on a per-device basis in the board `.dts`
  \end{itemize}


=== [fragile]{Resources: interrupts, clocks, DMA, reset lines, ...}
...
=== [fragile]{Pin-muxing controllers on STM32MP1}
  \begin{block}{#kfileversion("arch/arm/boot/dts/st/stm32mp151.dtsi", "6.1")}
{\tiny
\begin{minted}{perl}
...
=== [fragile]{Pin-muxing configuration}
\begin{onlyenv}<1>
  \begin{block}{#kfileversion("arch/arm/boot/dts/st/stm32mp15-pinctrl.dtsi", "6.1")}
{\tiny
\begin{minted}{perl}
&pinctrl {
        ...
...
\begin{onlyenv}<2>
  ...
  datasheet}. Note that `I2C1_SDA` is also available on pin `PF15` (not shown here).
\end{onlyenv}


=== [fragile]{Pin-muxing consumer}
  \begin{block}{}
{\tiny
\begin{minted}{perl}
&i2c1 {
...
\end{block}
\begin{itemize}
\item Typically board-specific, in `.dts`
\item `pinctrl-0`, `pinctrl-1`, `pinctrl-X` provides
  the pin mux configurations for the different {\bf states}
\item `pinctrl-names` gives a name to each state, mandatory even
  if only one state
\item States are mutually exclusive
\item The driver is responsible for switching between states
\item `default` state is automatically set up when the device is
  {\em probed}
\end{itemize}


=== [fragile]{Example: LED and I2C device}
  \begin{itemize}
  \item Let's see how to describe an LED and an I2C device connected
    to the DK1 platform.
  \item Create `arch/arm/boot/dts/st/stm32mp157a-dk1-custom.dts`
    which includes `stm32mp157a-dk1.dts`
    \begin{block}{}
{\tiny
\begin{verbatim}
...
\end{verbatim}
    ...
    \end{block}
  \item Make sure `stm32mp157a-dk1-custom.dts` gets compiled to a
...
=== [fragile]{Example: describe an LED}
...
=== [fragile]{Example: connect I2C temperature, humidity and pressure sensor}
...
\small Check out our {\em Device Tree 101 webinar}, by Thomas Petazzoni (2021)
\begin{itemize}
...
=== {Discoverable hardware}
  ...
  \item Useful Linux commands
    \begin{itemize}
    \item `lsusb`, lists all USB devices detected
    \item `lspci`, lists all PCI devices detected
...
#setuplabframe([Accessing hardware devices],[
  ...
  \begin{itemize}
  \item Exploring the contents of `/dev` and `/sys` and the
