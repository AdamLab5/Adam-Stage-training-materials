...
=== Two possibilities for the toolchain
    ...
    \item Buildroot offers two choices for the toolchain, called {\bf
      ...
      \end{itemize}
    \item Selected from ```Toolchain``` $\rightarrow$ ```Toolchain
        type```.
    \end{itemize}
    \column{0.4\textwidth}
...
=== Internal toolchain backend: result
  \begin{itemize}
  \item ```host/bin/<tuple>-<tool>```, the cross-compilation
    tools: compiler, linker, assembler, and more. The compiler is
    hidden behind a wrapper program.
  \item ```host/<tuple>/```
    \begin{itemize}
    \item ```sysroot/usr/include/```, the kernel
      headers and C library headers
    \item ```sysroot/lib/``` and ```sysroot/usr/lib/```, C library and
      gcc runtime
    \item ```include/c++/```, C++ library headers
    \item ```lib/```, host libraries needed by gcc/binutils
    \end{itemize}
  \item ```target/```
    \begin{itemize}
    \item ```lib/``` and ```usr/lib/```, C and C++ libraries
    \end{itemize}
