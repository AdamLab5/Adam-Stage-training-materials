...
[
      ...
      \item Follows a regular release schedule. Every 2 or 3 months,
        a new version is released. Versions are named `YYYY.MM`.
      \end{itemize}
    \column{0.3\textwidth}
...
=== {U-Boot configuration}
  ...
  \item Note: migration to {\em kconfig} is still on-going
    ...
    \item Many boards still have a combination of configuration
      ...
      and configuration settings in `defconfig` files
    \end{itemize}
  \end{itemize}


=== U-Boot configuration file: {\tt stm32mp15\_trusted\_defconfig}
  \begin{block}{}
    {\tiny
\begin{verbatim}
...
=== [fragile]{U-Boot configuration}
  
#table(columns: (50\%, 50\%), stroke: none,
[
    \begin{itemize}
    \item U-Boot must be configured before being compiled
    \item Configuration stored in a `.config` file
    \item Load a pre-defined configuration
\begin{verbatim}
\$ make BOARDNAME_defconfig
\end{verbatim}
      Where `BOARDNAME` is the name of a configuration, as
      visible in the #projdir("u-boot", "configs") directory.
    \item You can then run `make menuconfig` to further customize
...
=== [fragile]{U-Boot compilation}
  \begin{itemize}
  \item The path to the cross-compiler must be specified in the
    `CROSS_COMPILE` variable
  \item `CROSS_COMPILE` contains the prefix common to all
    cross-compilation tools, e.g `arm-linux-`
  \item Common to add the cross-compiler location in `PATH` to
    keep the `CROSS_COMPILE` value short
\begin{verbatim}
\$ export PATH=/path/to/toolchain/bin:\$PATH
\$ make CROSS_COMPILE=arm-linux-
\end{verbatim}
  \item The main result is a `u-boot.bin` file, which is the
    U-Boot image.
...
=== {Device Tree in U-Boot}
  ...
  \item Since v2024.07 the Device Tree files location depends on `CONFIG_OF_UPSTREAM`:
    \begin{itemize}
      \item `dts/upstream/src/ARCH/VENDOR` when `CONFIG_OF_UPSTREAM` is set
      \item `arch/ARCH/dts` otherwise
    \end{itemize}
  \item One `.dts` for each board: need to create one if you
    build a custom board
  \item U-Boot {\em defconfigs} usually specify a default Device Tree,
    but it can be overridden using the `DEVICE_TREE` variable
  \item More details on the {\em Device Tree} later in this course.
  \end{itemize}


=== [fragile]{U-Boot build example: TI AM335x BeagleBoneBlack wireless}
  \begin{itemize}
  \item One {\em defconfig} file suitable for all AM335x platforms:
    ...
    \item Yes its name looks like it supports only the EVM (EValuation
      Module) board
    \item Contains `CONFIG_DEFAULT_DEVICE_TREE="am335x-evm"`
      \$\rightarrow\$ uses
...
\end{verbatim}
  \item Produces:
    \begin{itemize}
    \item `MLO`, the SPL, first-stage bootloader. Called MLO ({\em
    Mmc LOad}) as required on TI platforms.
    \item `u-boot.img`, full U-Boot, second-stage bootloader
    \end{itemize}
  \end{itemize}
...
=== [fragile]{U-Boot {\tt help} command}
  
#table(columns: (50\%, 50\%), stroke: none,
[
    \begin{itemize}
    \item `help` command to list all available commands
    \item The set of available commands depend on the U-Boot
      configuration
      \begin{itemize}
      \item Many `CONFIG_CMD_*` options to enable commands at
        compile time
      \item See {\em Command line interface} submenu in
        `menuconfig`
      \end{itemize}
    \item `help <command>` for the complete help of one command
    \end{itemize}
    \column{0.5\textwidth}
    \begin{block}{}
      {\tiny
...
=== [fragile]{U-Boot information commands}

  
#table(columns: (50\%, 50\%), stroke: none,
[
    \begin{block}{Version details: `version`}
      {\tiny
\begin{verbatim}
...
\end{verbatim}
    ...
    \begin{block}{NAND flash information: `nand info`}
      {\tiny
\begin{verbatim}
...
\end{verbatim}
    ...
    \begin{block}{MMC information: `mmc info`}
      {\tiny
\begin{verbatim}
...
\end{verbatim}
    ...
    \begin{block}{Board information: `bdinfo`}
      {\tiny
\begin{verbatim}
...
\end{verbatim}
    ...
    {\scriptsize
      \begin{itemize}
      \item DRAM starts at `0xc0000000`, for a size of 512~MB
        (`0x20000000`).
      \item The end of the memory is used by U-Boot itself:
        `relocaddr` is the location of U-Boot in RAM.
      \end{itemize}
    }
  \\]
...
=== {U-Boot environment commands}
  \begin{itemize}
    \item `printenv`\\
      Shows all variables
    \item `printenv <variable-name>`\\
      Shows the value of a variable
    \item `setenv <variable-name> <variable-value>`\\
      Changes the value of a variable or defines a new one, only in RAM
    \item `editenv <variable-name>`\\
      Interactively edits the value of a variable, only in RAM
    \item After an `editenv` or `setenv`, changes in the
    ...
    \item `env` command, with many sub-commands: `env
        default`, `env info`, `env erase`, `env set`,
      `env save`, etc.
  \end{itemize}


=== [fragile]{U-Boot environment commands example}
  \begin{block}{}
\small
\begin{verbatim}
...
=== {U-Boot memory allocation}
  ...
  \item Use the output of `bdinfo` to know the start address and
    size of RAM
...
[
    ...
    \item NAND flash
      \begin{itemize}
      \item `nand info`
      \item `nand read <addr> <off|partition> <size>`
      \item `nand erase [<off> [<size>]]`
      \item `nand write <addr> <off|partition> <size>`
      \item More: `help nand`
      \end{itemize}
    \item MMC
      \begin{itemize}
      ...
      \item `mmc part` to show partition table
      \item `mmc dev` to show/set current MMC device
      \item More: `help mmc`
      \end{itemize}
    \end{itemize}
    \column{0.4\textwidth}
...
=== [fragile]{U-Boot commands example}
...
=== {U-Boot filesystem storage commands}
  ...
  \item Per-filesystem commands
    \begin{itemize}
    \item FAT: `fatinfo`, `fatls`, `fatsize`,
      `fatload`, `fatwrite`
    \item ext2/3/4: `ext2ls`, `ext4ls`, `ext2load`,
      `ext4load`, `ext4size`, `ext4write`
    \item Squashfs: `sqfsls`, `sqfsload`
    \end{itemize}
  \item ``New'' generic commands, working for all filesystem types
    \begin{itemize}
    \item Load a file: `load <interface> [<dev[:part]> [<addr> [<filename> [bytes [pos]]]]]`
    \item List files: `ls <interface> [<dev[:part]> [directory]]`
    \item Get the size of a file: `size <interface> <dev[:part]> <filename>`\\
	  (result stored in `filesize` environment variable)
    \item `interface`: `mmc`, `usb`
    \item `dev`: device number, `0` for first device, `1` for second device
    \item `part`: partition number
    \end{itemize}
  \end{itemize}


=== [fragile]{U-Boot filesystem command example}
...
c0000010: 756f6d3c 7020746e 3c093e74 65707974  <mount pt>.<type c0000020: 6f3c093e 6f697470 093e736e 6d75643c  >.<options>.<dum c0000030: 3c093e70 73736170 642f0a3e 722f7665  p>.<pass>./dev/r c0000040: 09746f6f 6509092f 09327478 6e2c7772  oot./..ext2.rw,n
[...]
\end{verbatim}
      }
    \end{block}
...
=== {U-Boot networking}
  ...
  \item Environment variables
    \begin{itemize}
    \item `ethaddr`: MAC address
    \item `ipaddr`: IP address of the board
    \item `serverip`: IP address of the server for network related commands
    \end{itemize}
  \item Important commands
    \begin{itemize}
    \item `ping`: ping a destination machine. Note: U-Boot is not
      an operating system with multitasking/interrupts, so ping from
      another machine to U-Boot cannot work.
    \item `tftp`: load a file using the TFTP protocol
    \item `dhcp`: get an IP address using DHCP
    \end{itemize}
  \end{itemize}
...
=== {TFTP}
  ...
  \item A TFTP server is needed on the development workstation
    \begin{itemize}
    \item `sudo apt install tftpd-hpa`
    \item All files in `/srv/tftp` are then visible through TFTP
    \item A TFTP client is available in the `tftp-hpa` package,
      for testing
    \end{itemize}
  \item A TFTP client is integrated into U-Boot
    \begin{itemize}
    \item Configure the `ipaddr`, `serverip`, and
      `ethaddr` environment variables
    \item Use `tftp <address> <filename>` to load file contents to
      the specified RAM address
    \item Example: `tftp 0x21000000 zImage`
    \end{itemize}
  \end{itemize}
...
=== {Scripts in environment variables}
  ...
  \item Environment variables can contain small scripts, to execute
    ...
    \item Several commands can be chained using the `;` operator
    \item Tests can be done using
      `if command ; then ... ; else ... ; fi`
    \item Scripts are executed using `run <variable-name>`
    \item You can reference other variables using
      `\${variable-name`}
    \end{itemize}
  \item Examples
    \begin{itemize}
    \item `setenv bootcmd 'tftp 0x21000000 zImage; tftp 0x22000000 dtb; bootz 0x21000000 - 0x22000000'`
    \item `setenv mmc-boot 'if fatload mmc 0 80000000
...
=== {U-Boot booting commands}
  ...
  \item Commands to boot a Linux kernel image
    \begin{itemize}
    \item `bootz` \$\rightarrow\$ boot a compressed ARM32 `zImage`
    \item `booti` \$\rightarrow\$ boot an uncompressed ARM64 or RISC-V `Image`
    \item `bootm` \$\rightarrow\$ boot a kernel image with legacy U-Boot headers
    \item `zboot` \$\rightarrow\$ boot a compressed x86 `bzImage`
    \end{itemize}
  \item `bootz [addr [initrd[:size]] [fdt]]`
    \begin{itemize}
    \item `addr`: address of the kernel image in RAM
    \item `initrd`: address of the {\em initrd} or {\em initramfs}, if any. Otherwise, must pass `-`
    \item `fdt`: address of the {\em Device Tree} passed to the Linux kernel
    \end{itemize}
  \item Important environment variables
    \begin{itemize}
    \item `bootcmd`: list of commands executed automatically by
      U-Boot after the count down
    \item `bootargs`: Linux kernel command line
    \end{itemize}
  \end{itemize}


=== [fragile]{U-Boot booting example}
...
=== {FIT image}
  ...
  \item Interestingly, relies on the {\em Device Tree Compiler}
    \begin{itemize}
    \item `.its` file describes the contents of the image
    \item Device Tree Compiler compiles it into an `.itb`
    \end{itemize}
  \item U-Boot can load an `.itb` image and use its different
    elements
...
=== {Generic Distro boot (1)}
  ...
  \item If enabled, at boot time, U-Boot:
    \begin{itemize}
    \item Can be asked to locate a bootable partition (`part list` command),
          as defined by the bootable flag of the partition table
    \item With the `sysboot` command, will look for a
...
[
  ...
  \begin{itemize}
     \item `kernel_addr_r`: address in RAM to load the kernel image
     \item `ramdisk_addr_r`: address in RAM to load the initramfs image (if any)
     \item `fdt_addr_r`: address in RAM to load the DTB (Flattened
           Device Tree)
     \item `pxefile_addr_r`: address in RAM to load the
           configuration file (usually `extlinux.conf`)
     \item `bootfile`: the path to the configuration file,
           for example `/boot/extlinux/extlinux.conf`
  \end{itemize}
  \column{0.5\textwidth}
