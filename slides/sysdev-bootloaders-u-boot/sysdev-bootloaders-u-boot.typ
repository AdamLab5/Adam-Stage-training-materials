...
#table(columns: (50%, 50%), stroke: none, [ U-Boot is a typical free
software project
...
- #strong[Ideal:] your platform is supported directly by
  #strong[upstream] U-Boot

  - Best quality \$\$\_code reviewed and approved by the community
...
- #strong[Less ideal:] use a #strong[fork] of U-Boot by your silicon
  vendor, system-on-module vendor or board vendor

  - Generally older, does not follow all upstream U-Boot updates

  - Changes not reviewed by the community \$\$\_quality is often dubious
...
- If designing your own custom board
  ...
  - If good support for your SoC in upstream U-Boot \$\$\_use upstream
    U-Boot

  - If not \$\$\_use the U-Boot fork from your SoC vendor

===  U-Boot configuration

- Configuration system based on #emph[kconfig] from the Linux kernel
...
- Note: migration to #emph[kconfig] is still on-going
...
```
CONFIG_ARM=y CONFIG_ARCH_STM32MP=y CONFIG_TFABOOT=y CONFIG_SYS_MALLOC_F_LEN=0x3000
CONFIG_ENV_OFFSET=0x280000
CONFIG_ENV_SECT_SIZE=0x40000
CONFIG_DEFAULT_DEVICE_TREE="stm32mp157c-ev1"
[..]
CONFIG_CMD_ADTIMG=y CONFIG_CMD_ERASEENV=y CONFIG_CMD_NVEDIT_EFI=y CONFIG_CMD_MEMINFO=y CONFIG_CMD_MEMTEST=y CONFIG_CMD_UNZIP=y CONFIG_CMD_ADC=y CONFIG_CMD_CLK=y CONFIG_CMD_DFU=y CONFIG_CMD_FUSE=y CONFIG_CMD_GPIO=y
[...]
CONFIG_SPI=y CONFIG_DM_SPI=y CONFIG_STM32_QSPI=y CONFIG_STM32_SPI=y
[...]
```
...
- Common to add the cross-compiler location in `PATH` to keep the
  ...
  ```
  \$\_export PATH=/path/to/toolchain/bin:\$PATH
...
- SPL = #emph[Secondary Program Loader]

- The SPL is a stripped-down version of U-Boot, made small enough to
  meet the size constraints of a first stage bootloader

- Configured through #emph[menuconfig], one can define the subset of
  drivers to include

- No U-Boot shell/commands: the behavior is hardcoded in C code

- For some platforms: TPL, #emph[Tertiary Program Loader], an even more
  minimal first stage bootloader to do TPL \$\$\_SPL \$\$\_main U-Boot.

===  Device Tree in U-Boot

- The #emph[Device Tree] is a data structure that describes the topology
  of the hardware
...
- U-Boot #emph[defconfigs] usually specify a default Device Tree, but it
  can be overridden using the `DEVICE_TREE` variable

- More details on the #emph[Device Tree] later in this course.

===  U-Boot build example: TI AM335x BeagleBoneBlack
wireless

- One #emph[defconfig] file suitable for all AM335x platforms:
  #projfile("u-boot", "configs/am335x_evm_defconfig")

  - Yes its name looks like it supports only the EVM (EValuation Module)
    board

  - Contains `CONFIG_DEFAULT_DEVICE_TREE="am335x-evm"` \$\$\_uses
    #projfile("u-boot", "arch/arm/dts/am335x-evm.dts") by default

- One #emph[Device Tree] file describing the BeagleBoneBlack Wireless:
...
- Configure and build U-Boot
  ...
  ```
  \$\_export PATH=/path/to/toolchain/bin:\$PATH
  \$\_make am335x_evm_defconfig
  \$\_make DEVICE_TREE=am335x-boneblack-wireless CROSS_COMPILE=arm-linux-
  ```

- Produces:

  - `MLO`, the SPL, first-stage bootloader. Called MLO ( #emph[Mmc
    LOad]) as required on TI platforms.
...
- The set of available commands depend on the U-Boot configuration
  ...
  - See #emph[Command line interface] submenu in `menuconfig`
...
#table(columns: (50%, 50%), stroke: none, [ Depending on the
configuration, the U-Boot environment can be:
...
=> printenv serverip serverip=10.0.0.100
=> saveenv
```
...
- No built-in memory allocation mechanism \$\$\_up to the user to know
...
===  U-Boot raw storage commands U-Boot can manipulate raw storage
devices:
...
```
STM32MP> mmc part Partition Map for MMC device 0  --   Partition Type: EFI
...
- "New" generic commands, working for all filesystem types
  ...
  - Load a file: `load <interface> [<dev[:part]> [<addr>
    [<filename> [bytes [pos]]]]]`

  - List files: `ls <interface> [<dev[:part]> [directory]]`

  - Get the size of a file: `size <interface> <dev[:part]>
    <filename>` 
...
c0000010: 756f6d3c 7020746e 3c093e74 65707974  <mountpt>.\<type c0000020: 6f3c093e 6f697470 093e736e 6d75643c  >.<options>.\<dum c0000030: 3c093e70 73736170 642f0a3e 722f7665  p>.<pass>./dev/r c0000040: 09746f6f 6509092f 09327478 6e2c7772  oot./..ext2.rw,n
[...]
```
...
- Network transfer from the development workstation to U-Boot on the
  ...
  - #emph[Trivial File Transfer Protocol]
...
- Examples
  ...
  - 'setenv mmc-boot `if fatload mmc 0 80000000 boot.ini; then source;
    else if fatload mmc 0 80000000 zImage; then run mmc-do-boot; fi;
    fi'`
...
- Commands to boot a Linux kernel image
  ...
  - `bootz` \$\$\_boot a compressed ARM32 `zImage`

  - `booti` \$\$\_boot an uncompressed ARM64 or RISC-V `Image`

  - `bootm` \$\$\_boot a kernel image with legacy U-Boot headers

  - `zboot` \$\$\_boot a compressed x86 `bzImage`

- `bootz [addr [initrd[:size]] [fdt]]`

  - `addr`: address of the kernel image in RAM

  - `initrd`: address of the #emph[initrd] or #emph[initramfs], if any.
    Otherwise, must pass `-`

  - `fdt`: address of the #emph[Device Tree] passed to the Linux kernel
...
- U-Boot has a concept of #strong[FIT] image

- FIT = #emph[Flat Image Tree]
...
- Interestingly, relies on the #emph[Device Tree Compiler]
...
- #emph[Generic Distro boot] concept
...
#table(columns: (50%, 50%), stroke: none, [ Several environment
...
```
  ...
  Found /boot/extlinux/extlinux.conf Retrieving file: /boot/extlinux/extlinux.conf
  131 bytes read in 143 ms (0 Bytes/s)
  1:      stm32mp157c-dk2-buildroot Retrieving file: /boot/zImage
  7538376 bytes read in 462 ms (15.6 MiB/s)
  append: root=/dev/mmcblk0p4 rootwait Retrieving file: /boot/stm32mp157c-dk2.dtb
  117969 bytes read in 148 ms (778.3 KiB/s)
  Kernel image @ 0xc2000000 [ 0x000000 - 0x7306c8 ]
  #Flattened Device Tree blob at c4000000
  Booting using the fdt blob at 0xc4000000
