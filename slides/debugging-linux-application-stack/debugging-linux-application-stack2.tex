...
=== Processes and Threads (1/2)
  \begin{itemize}
    ...
    \item A process is identified by a PID ({\bf P}rocess {\bf ID}) and all the
          ...
          ``` /proc/<pid> ```.
    \begin{itemize}
      \item A special file named ``` /proc/self ``` accessible by the process
...
=== Processes and Threads (2/2)
  \begin{itemize}
    ...
    \begin{itemize}
      \item Internally, both will call ``` clone() ``` with different flags
    \end{itemize}
...
#columns(gutter: 8pt)[
    \begin{itemize}
      \item Addresses accessed by the CPU are {\em virtual}
      \item The {\em Memory Management Unit} (MMU) translates them to {\em
        physical} addresses
      \item The kernel decides the translation and fills the {\em page
        table}
      \item The MMU reads the page table and caches recent entries in the
        {\em Translation lookaside buffers} (TLB) for zero-delay mapping
    \end{itemize}
     #colbreak()
    \includegraphics[width=\textwidth]{slides/debugging-linux-application-stack/mmu.pdf}
  \\]


=== MMU and memory management
  \begin{itemize}
    \item Physical addresses can be either RAM or I/O (to access devices)
    ...
    \item Page size is fixed and depends on the architecture/kernel
          configuration.
    \item Linux can work without an MMU (\kconfigval{CONFIG_MMU}{n}),
...
#columns(gutter: 8pt)[
    \begin{itemize}
      \item Each process has its own set of virtual memory areas (``` mm ```
      ...
      \begin{itemize}
        ...
        \item Can be disabled using ``` norandmaps ``` command line parameter
      \end{itemize}
    \end{itemize}
     #colbreak()
...
#columns(gutter: 8pt)[
    \begin{itemize}
      ...
      \item {\bf K}ernel {\bf A}ddress {\bf S}pace {\bf L}ayout
            ...
            layout.
      \begin{itemize}
        \item Can be disabled using ``` nokaslr ``` command line parameter
      \end{itemize}
    \end{itemize}
     #colbreak()
...
=== Userspace memory segments
  \begin{itemize}
    ...
    \end{itemize}
    \item New memory zones can be created using ``` mmap() ```
          (\manpage{mmap}{2})
...
=== Virtual memory VS physical memory
  ...
  \end{itemize}
  \center\includegraphics[height=0.7\textheight]{slides/debugging-linux-application-stack/memory_mapping.pdf}
...
=== On-demand memory mapping ({\em Lazy allocation)}
...
#columns(gutter: 8pt)[
    ...
    \end{itemize}
     #colbreak()
    \includegraphics[height=0.85\textheight]{slides/debugging-linux-application-stack/lazy_mapping.pdf}
  \\]


=== On-demand memory mapping: page faults
  \begin{itemize}
    \item Lazy allocation is implemented based on {\em Page faults}
    \begin{enumerate}
      \item The CPU accesses a virtual address, valid but not yet
        physically mapped
        \begin{itemize}
        \item E.g.: executes an instruction or reads/writes data
        \end{itemize}
      \item The MMU finds no mapping in the Page table $\rightarrow$ issues
        a Page fault {\em exception} to the CPU
      \item The CPU is immediately turned in kernel mode and executes an
        exception vector (a function provided by the kernel)
      \item The kernel checks if the virtual address is valid, finds a
        suitable physical page to map, fills the page (reads from disk if
        needed) and creates a page table entry
      \item The kernel continues execution normally
      \item The MMU now finds a mapping and continues
    \end{enumerate}
  \end{itemize}
...
\subsection{Execution mode switching}

=== Execution mode switching
  \begin{itemize}
    \item Execution mode switching is the action of changing the execution
	    mode of the processor (Kernel $\leftrightarrow$ User).
    \begin{itemize}
...
=== System Calls (1/2)
  \begin{itemize}
    ...
    \begin{itemize}
      \item When executing functions provided by the libc (``` read() ```,
    ...
    \item System calls are identified by a numeric identifier that is passed
          via the registers.
    \begin{itemize}
      \item The kernel exports some defines (in ``` unistd.h ```) that are named
...
=== Kernel threads
  \begin{itemize}
    ...
    \item These processes are cloned from the ``` kthreadd ``` process and can be
...
=== Threaded interrupts
  \begin{itemize}
    ...
    \item One kthread is created for each interrupt line that was requested as
          a threaded IRQ.
    \begin{itemize}
      \item {\em kthread} is named ``` irq/<irq>-<name> ``` and can be seen using {\em ps}.
    \end{itemize}
  \end{itemize}
...
=== Allocations and context
  \begin{itemize}
    ...
    \begin{itemize}
      \item ```c void *kmalloc(size_t size, gfp_t gfp_mask); ``` \item ```c void *kzalloc(size_t size, gfp_t gfp_mask); ``` \item ```c unsigned long __get_free_pages(gfp_t gfp_mask, unsigned int order) ```
    \end{itemize}
    \item All allocation functions take a ``` gfp_mask ``` parameter which allows to
