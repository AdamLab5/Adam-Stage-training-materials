...
- A process is identified by a PID (#strong[P]rocess #strong[ID]) and
...
- Addresses accessed by the CPU are #emph[virtual]

- The #emph[Memory Management Unit] (MMU) translates them to
  #emph[physical] addresses

- The kernel decides the translation and fills the #emph[page table]
...
- By default, all user mapping addresses are randomized to minimize
  ...
  - #strong[A]ddress #strong[S]pace #strong[L]ayout
    #strong[R]andomization
...
- #strong[K]ernel #strong[A]ddress #strong[S]pace #strong[L]ayout
  #strong[R]andomization also allows to randomize kernel address space
  layout.
...
+ When the CPU accesses a virtual address not yet physically mapped, a
  #emph[page fault] happens, and the kernel physically allocates memory
...
- Lazy allocation is implemented based on #emph[Page faults]
  ...
  + The MMU finds no mapping in the Page table $arrow.r$ issues a Page
    fault #emph[exception] to the CPU
...
- When using Linux tools, four terms are used to describe memory:
  ...
  - #emph[VSS/VSZ]: Virtual Set Size (Virtual memory size, shared
    libraries included).

  - #emph[RSS]: Resident Set Size (Total physical memory usage, shared
    libraries included).

  - #emph[PSS]: Proportional Set Size (Actual physical memory used,
    divided by the number of times it has been mapped).

  - #emph[USS]: Unique Set Size (Physical memory occupied by the
...
- The #emph[process context] can be seen as the content of the CPU
...
- Execution mode switching is the action of changing the execution mode
  of the processor (Kernel $arrow.l.r$ User).
...
- Interrupts are asynchronous signals that are generated by the hardware
  ...
  - Can also be synchronous when generated using a specific instruction
    (#strong[I]nter #strong[P]rocessor #strong[I]nterrupts for
    instance).
...
- One kthread is created for each interrupt line that was requested as a
  ...
  - #emph[kthread] is named ``` irq/<irq>-<name> ``` and can be seen
    using #emph[ps].
