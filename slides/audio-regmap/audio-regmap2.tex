...
#define regmap_init_spi(dev, config)                             \
  ...
  \end{itemize}
  \begin{itemize}
    \item Also ``` devm_ ``` versions
    \item and ``` _clk ``` versions, preparing, enabling and disabling
      clocks when necessary
  \end{itemize}


=== [fragile]{regmap: config}
...
=== [fragile]{regmap: config}
  \begin{itemize}
  \item ``` reg_bits ``` Number of bits in a register address, mandatory.
  \item ``` reg_stride ``` The register address stride. Valid register
    addresses are a multiple of this value. If set to 0, a value of 1
    will be used.
  \item ``` writeable_reg ```, ``` readable_reg ```,
    ``` volatile_reg ```, ``` precious_reg ```: Optional callbacks
  ...
  \item ``` reg_read ```, ``` reg_write ```, ``` reg_update_bits ```:
    Optional callbacks that if filled will be used to perform
    accesses. ``` reg_update_bits ``` should only be provided if
    specific locking is required.
  \item ``` reg_defaults ```: Power on reset values for registers (for
    use with register cache support).
  \item ``` num_reg_defaults ```: Number of elements in
    ``` reg_defaults ```.
  \end{itemize}


=== [fragile]{regmap: access}
  \begin{itemize}
    \fontsize{9}{9}\selectfont
    \item 
      \begin{block}{}
...
=== [fragile]{regmap: cache management}
  \begin{itemize}
    \item
      \begin{block}{}
        \begin{minted}{c}
...
=== [fragile]{regmap: i2c and spi device example}
...
=== [fragile]{regmap: ASoC components}
  \begin{itemize}
  \item ``` snd_soc_component ``` regmap accessors also exist, they are
    available either implicitly as the component core calls
    ``` dev_get_regmap(component->dev, NULL) ``` to retrieve or create
