...
- When downloading a file, Buildroot will successively try the following
  ...
  + The #strong[primary site], as indicated by `BR2_PRIMARY_SITE`

  + The #strong[original site], as indicated by the package `.mk` file

  + The #strong[backup Buildroot mirror], as indicated by
    `BR2_BACKUP_SITE`
...
- Some language-specific package management systems like to download the
  dependencies by themselves: #emph[vendoring]

- Examples: #emph[Cargo] in the Rust ecosystem, or #emph[Go]

- Problem for build systems: reproducibility of the builds, licensing,
  offline builds

- Buildroot supports vendoring dependencies for #emph[cargo] and
  #emph[go] packages
...
- Buildroot packages can provide a `.hash` file to provide #emph[hashes]
