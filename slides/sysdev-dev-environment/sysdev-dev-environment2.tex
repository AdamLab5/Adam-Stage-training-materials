...
=== Serial line communication program
  ...
  \item In this training session, we recommend using the simplest of
    them: {\em Picocom}
    \begin{itemize}
    \item Installation with `sudo apt install picocom`
    \item Run with `picocom -b BAUD_RATE /dev/SERIAL_DEVICE`.
    \item Exit with `[Ctrl][a] [Ctrl][x]`
    \end{itemize}
  \item `SERIAL_DEVICE` is typically
    \begin{itemize}
    \item `ttyUSBx` for USB to serial converters
    \item `ttySx` for real serial ports
    \end{itemize}
  \item Most frequent command: `picocom -b 115200 /dev/ttyUSB0`
  \end{itemize}
\end{columns}