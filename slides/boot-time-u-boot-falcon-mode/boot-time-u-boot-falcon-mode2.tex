...
=== {What U-Boot does (1)}
  ...
  \begin{itemize}
    \item {\em ATAGS} - The old way (before Device Tree)\\
	  ...
	  line (`bootargs`), the machine ID and other information for Linux in
	...
	\item U-Boot stores the Linux kernel command line
	      (`bootargs`) in the `chosen` section
	      in the Device Tree.
    \end{itemize}
  \end{itemize}
...
=== {What U-Boot does (2)}
  \begin{itemize}
    ...
    \end{itemize}
  \end{itemize}
  Using the `spl export` command in U-Boot, you can do such
...
=== {Falcon mode usage overview (1)}
   ...
   \begin{itemize}
     \item Recompile U-Boot with support for Falcon Mode
	   ...
	   for `spl export` (#projconfig("u-boot", "CONFIG_CMD_SPL")),
	   and for the way you want to boot.
     \item Also make sure that #projconfig("u-boot", "CONFIG_SPL_SIZE_LIMIT") is
	   set (find the SRAM size for your CPU, `0x10000` for
           SAMA5D36), otherwise, U-Boot won't complain when the SPL
	   is bigger.
     \item Build the kernel legacy `uImage` file from `zImage`
	   (see next slides)
     \item Set the kernel command line (`bootargs` environment variable)
     \item Load the `uImage`, initramfs (if any) and Device Tree images
	   to RAM as usual.
   \end{itemize}
...
=== {Falcon mode usage overview (2)}
   ...
   \begin{itemize}
     ...
     \item Save the exported data ({\em ARGS}) from RAM to storage, in {\em Flattened Device
	   ...
	   \begin{itemize}
		\item `fdtargsaddr`: location of {\em ARGS} in RAM
		\item `fdtargslen`: size of {\em ARGS} in RAM
	   \end{itemize}
     \item If supported by your board (code explanations given later),
	   set your `boot_os` environment variable to `yes/Yes/true/True/1`
...
make LOADADDR=0x20008000 uImage
          \end{verbatim}
	  \end{block}
      \item Or generate it using U-Boot's `mkimage` command:\\
	  \begin{block}{}
	  \begin{verbatim}
...
\small Your `board/<vendor>/<board>/<board>.c` file must at least\\
...
#columns(gutter: 8pt)[
   \begin{itemize}
     \item If supported by your board, hit the specified key on the
           serial console and back in U-Boot, disable the `boot_os`
	   environment variable. That's it.
     \item Otherwise, try to cause OS loading to fail. The easiest way
	   is to erase the kernel binary and if needed the `spl
	   export` output.
     \item If this doesn't work, re-compile and update the SPL without
	   ...
	   to always return `1`. This way, you don't lose your
	   configuration.
   \end{itemize}
    #colbreak()
      \begin{center}
...
#columns(gutter: 8pt)[
	...
	\begin{itemize}
	   \item A FAT partition is required to store the SPL file
	         (`boot.bin`). SAMA5D36 doesn't support an SPL file
	   ...
	   \item Caution: partition offsets should be a multiple of the {\em
		 segment} size, as indicated by the device's
	         `preferred_erase_size` attribute under
		 `/sys/bus/mmc/devices/`.
	\end{itemize}
   \\]
...
\end{verbatim}
	\item {\bf Caution}: the last argument of `mmc write` is a
...
#columns(gutter: 8pt)[
         Reference test
	\begin{itemize}
           \item Loading `zImage` and `dtb` from FAT through
                 `fatload` and using a zero `bootdelay`:\\
	         `setenv bootdelay 0`
	         `setenv bootcmd 'fatload mmc 0:1 0x21000000 zImage; fatload mmc 0:1 0x22000000; bootz 0x21000000 - 0x22000000'`
	   ...
	   \item Best result (using `grabserial`):\\
...
=== {Booting from raw NAND - Results and notes}
  ...
  \begin{itemize}
     \item Reference test
	\begin{itemize}
           \item To be fair, using a zero `bootdelay` and the exact `zImage` and `dtb` size:\\
	         `setenv bootdelay 0`
  ...
  \begin{itemize}
	...
	\end{itemize}
     \item If booting doesn't work, the easiest way is to add
	   `puts();` lines to trace strategic functions
...
=== {Issues and lessons learned (2)}
   \begin{itemize}
      \item {\em Limited automation}: while the `uImage` file can be updated
            automatically in the storage image, any change in the kernel
	    command line or Device Tree must go through the `spl export` command
      ...
      \item Falcon mode severely complicates the implementation of A/B updates,
	    ...
	    in the SPL.
      \item {\em Side note}: Found that U-Boot's `bootm` was
	    noticeably slower than `bootz` (+170 ms)
   \end{itemize}
...
#columns(gutter: 8pt)[
   \begin{itemize}
      ...
      \item Didn't try with what U-Boot calls the {\em Raw} kernel
            ...
            Assuming this corresponds to the `arch/arm/boot/Image`
