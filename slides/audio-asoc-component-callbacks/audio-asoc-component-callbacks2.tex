...
=== {``` hw_params ```}
  ...
  \item If the requested parameters cannot be supported by the
    hardware, the ``` hw_params ``` callback can return an error code
...
=== {``` snd_pcm_hw_params ```}
  ...
  \item Usually not accessed directly but through accessors:
    \begin{itemize}
    \item ``` params_channels ```: the number of channels
    \item ``` params_rate ```: the sample rate
    \item ``` params_width ```: the number of bits per sample
    \end{itemize}
  \end{itemize}


=== [fragile]{``` hw_params ``` example}
  \begin{block}{#kfile("sound/soc/codecs/tlv320aic31xx.c")}
...
=== {``` set_tdm_slot ```}
  ...
  \item ``` slots ``` is the total number of slots of the TDM stream and
    ``` slot_width ``` the width of each slot in bit clock cycles.
  \item ``` tx_mask ``` and ``` rx_mask ``` are bitmasks specifying the
  ...
  \item This allows to explicitly configure mismatching stream and bus
    sample width.
  \item TDM mode must be disabled when ``` slots ``` is 0.
  \end{itemize}
...
=== [fragile]{``` trigger ``` example}
  \begin{block}{#kfile("sound/soc/codecs/pcm1789.c")}
    \fontsize{8}{8}\selectfont
    \begin{minted}{c}
...
=== {``` set_bias_level ```}
  ...
  \item Available events are:
    \begin{itemize}
    \item ``` SND_SOC_BIAS_ON ```: Bias is fully on for audio playback
      and capture operations.
    \item ``` SND_SOC_BIAS_PREPARE ```: Prepare for audio operations.
      Called before DAPM switching for
      stream start and stop operations.
    \item ``` SND_SOC_BIAS_STANDBY ```: Low power standby state when no
      playback/capture operations are
      in progress. NOTE: The transition time between STANDBY and ON
      should be as fast as possible and no longer than 10ms.
    \item ``` SND_SOC_BIAS_OFF ```: Power Off. No restrictions on
      transition times.
    \end{itemize}
  \end{itemize}
...
=== [fragile]{``` set_bias_level ``` example}
  \begin{block}{#kfile("sound/soc/codecs/ssm2518.c")}
    \fontsize{8}{8}\selectfont
    \begin{minted}{c}
