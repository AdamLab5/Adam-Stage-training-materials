...
=== {systemd (2)}
  ...
  \item Systemd also provides
    ...
    \item {\em udevd}, hotplugging and `/dev` management
...
=== [fragile]{systemd service unit file example}
...
=== {Example systemctl/journalctl commands}
  \begin{itemize}
  \item `systemctl status`, status of all services
  \item `systemctl status <service>`, status of one
    service
  \item `systemctl [start|stop] <service>`, start or stop a service
  \item `systemctl [enable|disable] <service>`, enable
    or disable a service, i.e. whether it should start at boot time
  \item `systemctl list-units`, list all available units
  \item `journalctl -a`, all logs
  \item `journalctl -f`, show the last entries, and keep printing
    new entries as they arrive
  \item `journalctl -u`, logs from a particular service
  \end{itemize}
...
=== {Display controller support}
  ...
  \item Modern Linux kernel subsystem: {\em DRM}
    ...
    \item Devices exposed as `/dev/dri/cardX`
    \item Companion user-space library: `libdrm`, includes a very
      handy test tool: `modetest`
...
=== {GPU support: OpenGL acceleration}
  ...
  \item Open-source
    ...
    \item `mesa3d` user-space library implementing the various
