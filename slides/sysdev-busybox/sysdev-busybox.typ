...
- Rewrite of many useful UNIX command line utilities
  ...
  - Called the #emph[Swiss Army Knife of Embedded Linux]
...
#table(columns: (50%, 50%), stroke: none, [ You can choose:
...
- Set the installation directory in the configuration interface: 
  `Settings -> Installation Options` ' -> Destination path for `make
  install'`

- Add the cross-compiler path to the PATH environment variable: 
  `export
  PATH=\$H O M E slash x minus t o o l s slash a r m minus u n k n o w n minus l i n u x minus u c l i b c g n u e a b i slash b i n colon\$PATH`
...
- Simpler than the init implementation found on desktop/server systems
  (#emph[SysV init] or #emph[systemd])
