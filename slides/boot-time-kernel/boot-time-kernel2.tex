...
=== Advice for kernel optimizations
	...
	\item During this phase, we recommend to switch to loading the kernel through
	      the network (`tftp`) if possible. This will save a lot of time
	...
	\item Loading the kernel through the network will introduce delays and jitter,
	      ...
	      \item If you want to measure the boot time impact of your changes,
		    you can still start counting time from the `Starting kernel`
		    message.
	      \end{itemize}
...
=== Measure - Kernel initialization functions To find out which kernel initialization functions are the longest to execute, add `initcall_debug` to the kernel command line.
...
#kconfig("CONFIG_LOG_BUF_SHIFT") in your kernel configuration. You will also need #kconfig("CONFIG_PRINTK_TIME") and #kconfig("CONFIG_KALLSYMS").


=== Kernel boot graph With `initcall_debug`, you can generate a boot graph making it easy to see which kernel initialization functions take most time to execute.
\begin{itemize}
\item Copy and paste the output of
      the `dmesg` command to a file (let's call it `boot.log`)
\item On your workstation, run the `scripts/bootgraph.pl` script
      in the kernel sources: \\
      `scripts/bootgraph.pl boot.log > boot.svg`
\item You can now open the boot graph with a vector graphics
      editor such as `inkscape`:
\end{itemize}
\begin{center}
...
\item Be careful: some function names don't exist, the names
      correspond to {\em modulename}`_init`. Then, look for
...
\item Optimize necessary functionality:
      ...
      \item Look for delay loops and calls to functions containing
            `delay` in their name, which could take more time than
...
=== Reduce kernel size First, we focus on reducing the size without removing features
\begin{itemize}
...
\begin{itemize}
	...
	\item Then study the corresponding `Makefile` to see
...
=== Kernel Compression Depending on the balance between your storage reading speed and your CPU power to decompress the kernel, you will need to benchmark different compression algorithms.
...
=== Kernel compression options Results on TI AM335x (ARM), 1 GHz, Linux 5.1
{\fontsize{7}{10}\selectfont
\begin{tabular}{| l || c | c | c | c | c |}
\hline Timestamp & gzip & lzma & xz & lzo & lz4 \\
\hline Size & 2350336 & 1777000 & {\bf 1720120} & 2533872 & 2716752 \\
...
\newline\newline Lzo and Gzip seem the best solutions. Always benchmark as the results depend on storage and CPU performance.
...
\begin{itemize}
    ...
    \item On U-Boot on ARM, you won't be able to boot with the
	  `bootz` command. You will need to use `bootm`
	  and a `uImage` file.
\end{itemize}
...
\hline
& O2 & Os & Diff \\
\hline Size & 7372432 & 6594440 & -10.5 \% \\
...
\item Your kernel will not shrink but some initializations will be
      postponed.
\item Typically, you would modify `probe()` functions to return
...
\item Console output is actually taking a lot of time (very slow device).
      ...
      passing the `quiet` argument on the kernel command line.
\item You will still be able to use `dmesg` to get the kernel
      messages.
\item Time between starting the kernel and starting the `init`
...
\item Don't do it too early if you're using `grabserial`
\end{itemize}
...
\begin{itemize}
        \item At each boot, the Linux kernel calibrates a delay loop (for
              ...
              the `lpj` value in the kernel boot messages:
\begin{block}{}
\small
\begin{verbatim}
...
\end{verbatim}
\end{block}
        \item Now, you can add `lpj=<value>` to the kernel command
              line:
\begin{block}{}
\small
\begin{verbatim}
...
\begin{itemize}
        \item #kconfigval("CONFIG_PRINTK", "n") will have the same effect as the
              `quiet` command line argument but you won't have
...
\begin{itemize}
\item Use `initcall_debug` to find the biggest
      time consumers
