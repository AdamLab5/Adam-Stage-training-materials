...
[
    ...
    \item Good for small to medium size embedded systems, with a fixed set of
      features
      \begin{itemize}
      \item No support for generating packages (`.deb` or
        `.ipk`)
...
[
    \begin{itemize}
    \item Configuration takes place through a `*config` interface similar to the
      kernel\\
      `make menuconfig`
    ...
    \item Build:\\
      `make`
    \item Useful build results in `output/images/`
    \end{itemize}
    \column{0.4\textwidth}
...
=== {Buildroot: adding a new package}
  ...
  \item Each package has its own directory (such as
    `package/jose`). This directory contains:
    \begin{itemize}
    \item A `Config.in` file (mandatory), describing the
      configuration options for the package. At least one is needed to
      enable the package. This file must be sourced from
      `package/Config.in`
    \item A `jose.mk` file (mandatory), describing how the
      package is built.
    \item A `jose.hash` file (optional, but recommended), containing
      hashes for the files to download, and for the license file.
    \item Patches (optional). Each file of the form
      `*.patch` will be applied as a patch.
    \end{itemize}
  \end{itemize}


=== [fragile]{Buildroot: adding a new package, Config.in}
...
=== [fragile]{Buildroot: adding new package, {\tt .mk} file}
...
\end{verbatim}
    ...
    \item The package directory and the prefix of all variables must be
      ...
      `BR2_PACKAGE_JOSE`
    \item The `meson-package` infrastructure knows how to build {\em
...
=== {Yocto Project concepts}
  ...
  \item Yocto/OpenEmbedded generate a {\em distribution}
    ...
    \item The other packages can be installed at runtime on the system
      using a package management system: `apt`, `dnf`,
      `opkg`
    \end{itemize}
  \end{itemize}
...
=== [fragile]{Yocto quick start: STM32MP1 example}
...
\end{verbatim}
  ...
  This automatically enters a directory called `build/`, with a
...
\end{verbatim}
  ...
  \begin{itemize}
  \item `MACHINE=stm32mp1` will build images usable on all
    STM32MP1 platforms
  \item `core-image-minimal` builds a minimal image
  \end{itemize}
