...
===  Getting started with #emph[meson]
...
===  Library dependencies with #emph[meson]
...
- In-kernel #emph[tracing] functionality

- Can trace

  - Well-defined trace locations in the kernel, called
    #emph[tracepoints], identifying important events in the kernel:
...
- Accessible using the dedicated #emph[tracefs] filesystem

- `trace-cmd` is a higher-level CLI tool to use #emph[ftrace]
...
- Visualization tool for #emph[ftrace] traces

- #link("https://kernelshark.org/")

#align(center, [#image("kernelshark.png", height: 60%)])

===  perf

- #emph[instrument CPU performance counters, tracepoints, kprobes, and
  uprobes]
...
- Profiling: sample on-CPU functions for the specified command, at 99
  Hertz 
...
#align(center, [#image("gprof2dot.pdf", height: 60%)])

Generated with
#link("https://github.com/jrfonseca/gprof2dot")[gprof2dot]
...
#align(center, [#image("cloud_word.png", height: 60%)])
...
- Using #emph[strace], #emph[ltrace], #emph[gdbserver] and #emph[perf]
