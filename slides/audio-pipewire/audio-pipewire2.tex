...
=== {Introduction}
  ...
  \item Same abstraction layer (alternatives)
    ...
    \end{itemize}
  \item Technical stack: C (``` gnu11 ```), Meson \& Ninja
  \end{itemize}




=== [fragile]{Concepts — objects}

  \begin{itemize}
  \item The graph state representation is a list of objects.
  \item That object list is handled by the ``` Core ``` object, hosted
    by the PipeWire daemon.
  \item Each connected process is represented by a ``` Client ``` object.
  \end{itemize}
...
=== {Concepts — nodes, ports \& links (1)}
  ...
  \item The graph itself is represented by the following object types:
    ...
    \begin{itemize}
    \item A ``` Node ``` processes samples
    \item A ``` Port ``` represents a node input or output
    \item A ``` Link ``` connects an output port with an input port
    \end{itemize}
...
=== [fragile]{Concepts — nodes, ports \& links (2)}
...
=== [fragile]{Concepts — object properties and params}
...
=== {Concepts — devices}
  ...
  \item Another object type is ``` Device ```. Those map to physical
...
=== {Concepts — graph execution logic (1)}
  ...
  \item Each node keeps two counters:
    \begin{enumerate}
    \item ``` required ```: the number of dependencies on other nodes;
    \item ``` pending ```: how many remaining nodes need to be executed
      before it can run in this cycle. A value of zero means the node
      can be executed. Its reset value is ``` required ```.
    \end{enumerate}
...
=== {Concepts — graph execution logic (2)}
  \begin{itemize}
  \item The driver node is picked based on the ``` priority.driver ``` property.
...
=== {Concepts — modules}
  ...
  \item Example modules:
    ...
    \begin{itemize}
    \item ``` module-rt ```: requests realtime scheduling priority using
      ``` setpriority(2) ``` and ``` pthread_setschedparam(3) ```.
    \item ``` module-loopback ```: create two virtual loopback nodes.
    \item ``` module-protocol-native ```: implements the communication
      between the daemon and clients.
    \item ``` module-profiler ```: implements the profiling logic,
...
=== {PipeWire communication protocols — IPC}
  ...
  \item ``` socket(AF_UNIX, SOCK_STREAM, 0) ``` for communication with
    the daemon process. The socket is named ``` pipewire-0 ``` by
    default or ``` $PIPEWIRE_REMOTE ```. Directory look-up order:
    \begin{enumerate}
    \item ``` $PIPEWIRE_RUNTIME_DIR ```
    \item ``` $XDG_RUNTIME_DIR ```
    \item ``` $USERPROFILE ```
    \end{enumerate}
  \item ``` eventfd(2) ``` is the wakeup method.
  \item ``` memfd_create(2) ``` is used for sharing multimedia data
  ...
  \item PipeWire provides an event-loop implementation that relies
    upon ``` epoll(7) ```. All clients use it. They also use
    ``` signalfd(2) ``` to handle signals.
...
=== {PipeWire communication protocols — D-Bus optional dependency}
  ...
  \item Flatpak permission support through
    ...
    XDG Desktop Portal}, see ``` libpipewire-module-portal ```
...
=== {Configuration — location (2)}
  ...
  \item A client that loads a config file named ``` client-rt.conf ```
    ...
    \item ``` $sysconfdir/pipewire/client-rt.conf.d/ ```
...
=== [fragile]{Configuration — sections (1)}

  \begin{itemize}
  \item ``` context.properties ``` configures the PipeWire instance.

  \item Most properties target the daemon
    (``` default.clock.allowed-rates ```,
    ``` default.clock.max-quantum ```, etc.) but some also apply to
    other clients (``` log.level ```, ``` mem.mlock-all ```, etc.).
  \end{itemize}
...
=== [fragile]{Configuration — sections (2)}

  \begin{itemize}
  \item ``` context.spa-libs ``` maps plugin features with globs to a SPA
    library.
...
=== [fragile]{Configuration — sections (3)}

  \begin{itemize}

  \item ``` context.modules ``` is an array of dictionaries. It lists
    modules to instantiate, with optional arguments (``` args ```),
    ``` flags ``` and a conditional expression (``` condition ```).
  ...
  \item Two flags exist to turn panics into warnings:
    ...
    \begin{enumerate}
    \item ``` ifexists ``` on unknown modules;
    \item ``` nofail ``` on module init failures.
    \end{enumerate}
...
=== [fragile]{Configuration — sections (4)}

  \begin{itemize}
  \item ``` context.modules ``` example:
  \end{itemize}
...
=== [fragile]{Configuration — sections (5)}

  \begin{itemize}

  \item ``` context.objects ``` is an array of dictionaries. It lists
    objects that should be statically created by this client. This
    requires a ``` factory ``` to be used and arguments (``` args ```) to
    be passed to it.

  \item As previously, the ``` flags ``` property can configure the
    reaction to errors. For ``` context.objects ```, only ``` nofail ```
    exists.

  \item ``` condition ``` also exists for this section.
...
=== [fragile]{Configuration — sections (6)}

  \begin{itemize}
  \item ``` context.objects ``` example:
  \end{itemize}
...
=== [fragile]{Configuration — sections (7)}

  \begin{itemize}

  \item ``` context.exec ``` is an array of dictionaries. Each entry
...
=== {Tools rundown — the ``` PIPEWIRE_DEBUG ``` variable}
  ...
  \item Every client listens to the ``` PIPEWIRE_DEBUG ``` environment
    variable which allows overwriting the ``` log.level ``` from the
    configuration file.

  \item It takes as value the log level:

    \begin{itemize}
    \item ``` 0 ``` or ``` X ```: No logging is enabled.
    \item ``` 1 ``` or ``` E ```: Error logging is enabled.
    \item ``` 2 ``` or ``` W ```: Warnings are enabled.
    \item ``` 3 ``` or ``` I ```: Informational messages are enabled.
    \item ``` 4 ``` or ``` D ```: Debug messages are enabled.
    \item ``` 5 ``` or ``` T ```: Trace messages are enabled.
    \end{itemize}
  ...
  \item This should be \textbf{the first debugging step} to increase
    ...
    facing issues. Careful with ``` PIPEWIRE_DEBUG=5 ``` which most
...
=== [fragile]{Tools rundown — ``` pw-config ```}
  \begin{itemize}

  \item ``` pw-config ``` is a small utility that allows dumping a given
  ...
  \item ``` pw-config paths ``` lists config paths, including overrides.

  \item ``` pw-config list ``` details all config sections.
...
=== {Tools rundown — ``` pw-dump ```}
  ...
  \item ``` pw-dump ``` prints the graph as a JSON array of all exported
    objects known to ``` Core ```.

  \item Its main goal is to allow sharing the graph’s overall state
    when reporting a bug or describing a situation.

  \item Filtering: ``` pw-dump ``` takes a parameter which can be an
    object type (careful, it must be capitalised), ID or name (
    ``` object.path ```, ``` object.serial ``` or ``` *.name ```).
...
=== {Tools rundown — ``` pw-cli ``` (1)}
  ...
  \item ``` pw-cli ``` is the main command-line interface tool to
...
=== {Tools rundown — ``` pw-cli ``` (2)}
  ...
  \item It can expose many information about the graph:
    ...
    \item ``` pw-cli ls [<filter>] ``` lists objects with their ID,
      type and a few of their core properties. ``` <filter> ``` is the
      same as ``` pw-dump ```'s argument.

    \item ``` pw-cli info <filter> ``` gives all possible
      information about a given object. That includes all of its
      properties and params.

    \item ``` pw-cli enum-params <filter> <param-id> ``` gives the
  ...
  \item But, it also allows modifying objects:
    ...
    \begin{itemize}
    \item ``` pw-cli set-param <filter> <param-id> <param-json> ``` to
      edit a param value;
    \item ``` pw-cli permissions <client-id> <object> <permission> ```
      to modify permissions on a given object.
    \end{itemize}

  \item As well as creating objects dynamically, that will be hosted by
    the ``` pw-cli ``` client: ``` load-module ```, ``` create-device ```,
...
=== {Tools rundown — ``` pw-profiler ```}
  ...
  \item Here is an example with a single ``` pw-play ``` node, first
    started with ``` PIPEWIRE_CONFIG_NAME ``` equal to
    ``` client.conf ``` then with ``` client-rt.conf ``` on a loaded
    system.
...
=== {Tools rundown — ``` pw-dot ```}
  ...
  \item ``` pw-dot ``` creates a file named ``` pw.dot ``` which is a
  ...
  \item By default, it connects to the PipeWire daemon and creates a
    ...
    the output of ``` pw-dump ``` using the ``` --json ``` flag.

  \item That file can be turned into a graphical representation and
    viewed on a host using:\\``` dot -Tsvg pw.dot > pw.svg && xdg-open pw.svg ```
...
=== {Tools rundown — ``` pw-cat ```}
  ...
  \item Aliased to ``` pw-play ```, ``` pw-record ``` and others, it is a
  ...
  \item It has many options available to control the exposed props and
    ...
    \begin{itemize}
    \item ``` --target ``` allows asking to be routed to a given node;
    \item ``` --latency ``` asks for a given latency (therefore buffer size);
    \item ``` --quality ``` controls the adaptive resampling;
    \item ``` --rate ```, ``` --channels ```, ``` --channel-map ```,
      ``` --format ```, ``` --volume ``` are self-describing.
    \item etc.
    \end{itemize}
...
=== {Tools rundown — and a few others}
  ...
  \item ``` pw-link ```: it allows listing, creating and deleting links.

  \item ``` pw-mon ```: it monitors and dumps various events: it prints
    when a global object is added or removed, displays information
    relative to the ``` Core ```, etc.

  \item ``` pw-loopback ```: it creates two nodes that act as a virtual
    loopback.

  \item ``` pw-metadata ```: it allows editing metadata, which are
...
=== [fragile]{Tools rundown — ``` helvum ``` (3)}
...
=== {Demo 1 — introduction}
  ...
  \item We will play audio to an ``` alsa-lib ``` device from an audio
    file.
  ...
  \item The steps will be:
    ...
    \item Start a ``` pw-play ``` client;
    \item Study the graph status using various tools (``` pw-dot ```,
      ``` pw-top ```, ``` pw-cli ```, etc).
    \end{enumerate}
...
=== [fragile]{Demo 1 — pointers}
  \begin{enumerate}

  \item Start a PipeWire daemon.

    \begin{itemize}
    \item Running ``` pipewire ``` without arguments will start a client
      using ``` pipewire.conf ```, which by default runs in daemon mode.
  ...
  \item Start a WirePlumber daemon.
    ...
    \item Its log level is controlled using ``` WIREPLUMBER_DEBUG ```.
    \end{itemize}
...
=== {Demo 2 — introduction}
  ...
  \item The steps will be:
    ...
    \item Update links manually to make ``` pw-play ``` be routed to the
...
=== [fragile]{Demo 2 — pointers}
  \begin{enumerate}

  \item To create a new configuration file, for the client hosting the effect.

    \begin{itemize}
    \item Recent PipeWire versions have a ``` filter-chain.conf ```
    ...
    \item When modules spawn objects, they often give their own
      ...
      properties for each node. See ``` capture.props ``` and
      ``` playback.props ```.
    \end{itemize}
  ...
  \item Start a client using this config.
    ...
    \begin{itemize}
    \item ``` pipewire -c filter-chain.conf ```
    \end{itemize}
  ...
  \item Update links manually to make ``` pw-play ``` be routed to the
    ...
    \item Otherwise,
      ``` pw-dot ``` or ``` pw-link --links ``` to get an overview then
...
=== {WirePlumber — default behavior}
  ...
  \item It enumerates and adds ``` Device ``` objects for ALSA, BlueZ
  ...
  \item Audio routing is based on two default nodes:
    ...
    \begin{itemize}
    \item An ``` Audio/Sink ``` node is for applications that want to
      emit audio. All ``` Output/Audio ``` nodes get routed to it.
    \item An ``` Audio/Source ``` node is for applications that require a
  ...
  \item Nodes can also request to be routed to:
    ...
    \begin{enumerate}
    \item a target node using ``` target.object ``` (for example
      ``` pw-cat --target ```);
    \item nothing automatically using ``` node.autoconnect ```.
      WirePlumber will not create any automatic link, letting any
      PipeWire client create the desired links;
    \item a target node using ``` target.object ``` inside ``` default ```
      metadata (at runtime).
    \end{enumerate}
...
=== {WirePlumber — configuration (1)}
  ...
  \item The default configuration is called ``` wireplumber.conf ```, see
...
=== [fragile]{WirePlumber — configuration (2)}
  \begin{itemize}
  \item Let's look at the configuration of the ALSA monitor
    in ``` scripts/monitors/alsa.lua ```:
  \end{itemize}
...
=== [fragile]{WirePlumber — configuration (3)}
  \begin{itemize}
...
=== [fragile]{WirePlumber — configuration (4)}
  \begin{itemize}
...
=== {WirePlumber — permission handling}
  ...
  \item That is handled, in PipeWire >= 0.3.83, using two PipeWire daemon sockets:
    ...
    \begin{itemize}
    \item Clients joining ``` pipewire-0-manager ``` have full permissions, seen
      using property ``` pipewire.access = "unrestricted" ```.
    \item Client joining ``` pipewire-0 ``` must be given permissions by the
...
=== {WirePlumber — DSP filtering on sinks and sources (1)}
  ...
  \item Script implementing this behavior is ``` scripts/node/software-dsp.lua ```.
...
=== [fragile]{WirePlumber — DSP filtering on sinks and sources (2)}
  \begin{itemize}
...
=== {WirePlumber — demo time! (2)}
  ...
  \item Target a specific node.
    ...
    \begin{itemize}
    \item This is done by nodes using ``` target.object ``` (previously
      ``` node.target ```).
  ...
  \item Modify the default playback node.
    ...
    \begin{itemize}
    \item ``` wpctl set-default <id> ``` controls this.
    \item Nodes must have ``` media.class ``` equal to ``` Audio/Sink ```
  ...
  \item Have a look at device profiles.
    ...
    \begin{itemize}
    \item Those are params on the device objects. See ``` EnumProfile ```
      and ``` Profile ```.
    \end{itemize}
...
=== [fragile]{C API — introduction}
  \begin{itemize}

  \item ``` libpipewire ```: reference implementation, and currently the only one.
  ...
  \item See ``` pkg-config ``` for CFLAGS and LDFLAGS:
...
=== [fragile]{C API — event-loop}
  \begin{itemize}
  \item At the core of each client: an ``` epoll(2) ```-based event-loop
    is running.
  \item ``` pw_main_loop ``` is a wrapper around ``` pw_loop ```
...
=== [fragile]{C API — context}
  \begin{itemize}

  \item A ``` pw_context ``` instance is at the heart of the C API. It
...
=== {C API — proxies}
  ...
  \item A client starts with two proxies:
    ...
    \begin{enumerate}
    \item One pointing to the ``` Core ``` object.
    \item Another one to the global ``` Client ``` object that represents
      itself.
    \end{enumerate}
...
=== [fragile]{C API — example 1, monitoring global objects}
  \begin{block}{}
    \fontsize{8}{8}\selectfont
      \begin{minted}{c}
...
=== {C API — node implementations}
  ...
  \item PipeWire provides two abstractions for implementing nodes:
    ...
    \item ``` pw_filter ```: DSP-type work, works on raw ``` f32 ```
      samples, without additional buffering.

    \item ``` pw_stream ```: more high level, it provides the following
      features:
      \begin{itemize}
  ...
  \item See example implementations of source nodes:
    \begin{itemize}
    \item Filter: ``` src/examples/audio-dsp-src.c ```
    \item Stream: ``` src/examples/audio-src.c ```
    \end{itemize}
...
=== [fragile]{C API — ``` pw_filter ``` process event}
...
=== [fragile]{C API — ``` pw_stream ``` process event}
...
=== {Going further}
  ...
  \item For MIDI support, see ``` pw-cat --midi ```, ``` pw-mididump ```
  ...
  \item For the JACK compatibility layer, look at ``` pw-jack ```.

  \item For video support, see many examples in ``` src/examples/ ```.

  \item For audio over IP, see modules ``` roc-* ```, ``` pulse-tunnel ```,
    ``` netjack2-* ```, ``` rtp-* ```, ``` protocol-simple ```, ``` avb ```.

  \item To understand why timer-based audio scheduling (``` tsched ```) is
    useful, see
