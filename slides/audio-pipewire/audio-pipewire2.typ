...
#columns(gutter: 8pt)[ Example with ``` pw-play audio.wav ``` and ``` pw-record –target pw-play rec.wav ```:
```text
$ pw-cli ls Core
...
- Objects also contain #strong[properties]: a list of string key-value
pairs. Those can only be modified by the client hosting the node.
- Some object types also contain #strong[params]. Those might be
...
those, there is exactly one #strong[driver] node, and zero or more
#strong[follower] nodes.
...
it can run in this cycle. A value of zero means the node can be executed. Its reset value is ``` required ```.
- Nodes also keep a list of nodes that depend on them (called targets); a node is responsible for decrementing its targets’ ``` pending ```
counters and signal them using IPC.
- See
#link("https://docs.pipewire.org/page_scheduling.html")[the documentation] for more details. The graph evaluation is implemented by ``` pw_context_recalc_graph() ```.
...
- #strong[Virtual loopbacks] are such an example: they allow sending
...
compatibility layer).
- The #strong[rate] is similar: it can be different for each subgraph.
...
#link("https://docs.flatpak.org/en/latest/desktop-integration.html#portals")[ XDG Desktop Portal], see ``` libpipewire-module-portal ```
- Audio device reservation through the
#link("https://git.0pointer.net/reserve.git/tree/reserve.txt")[ org.freedesktop.ReserveDevice1], see ``` libwireplumber-module-reserve-device ```
- For Bluetooth support through #link("https://www.bluez.org/")[BlueZ], see PipeWire’s ``` libspa-bluez5 ```
== Configuration
<configuration>
...
- Look-up order:
+ ``` $XDG_CONFIG_HOME/pipewire/ ``` environment variable, often ``` ~/.config/pipewire/ ``` in distributions item ```$sysconfdir/pipewire/ ```  compile-time variable, often ``` /etc/pipewire/ ```
+ ``` $datadir/pipewire/ ``` compile-time variable, often ``` /usr/share/pipewire/ ```
end{enumerate}
end{itemize}
...
begin{enumerate} item ```$datadir/pipewire/client-rt.conf.d/ ```
+ ``` $sysconfdir/pipewire/client-rt.conf.d/ ``` item ```$XDG_CONFIG_HOME/pipewire/client-rt.conf.d/ ```
...
- Using this section is therefore #strong[deprecated], except for simple
...
== Tools rundown
<tools-rundown> ===  Tools rundown — the ``` PIPEWIRE_DEBUG ``` variable
- Every client listens to the ``` PIPEWIRE_DEBUG ``` environment variable which allows overwriting the ``` log.level ``` from the configuration
file.
...
and therefore better understand why a PipeWire client is facing issues. Careful with ``` PIPEWIRE_DEBUG=5 ``` which most likely will
...
- It has two modes: (1) it can either answer to commands given as argument such as ``` pw-cli help ``` and stop afterwards or (2) run in
...
representation of the global objects. It can also work from the output of ``` pw-dump ``` using the ``` –json ``` flag.
- That file can be turned into a graphical representation and viewed on
a host using:  ``` dot -Tsvg pw.dot > pw.svg && xdg-open pw.svg ```
...
global object is added or removed, displays information relative to the ``` Core ```, etc.
...
+ Study the graph status using various tools (``` pw-dot ```, ``` pw-top ```, ``` pw-cli ```, etc).
===  Demo 1 — pointers
+ Start a PipeWire daemon.
...
children, and take arguments to set specific properties for each node. See ``` capture.props ``` and ``` playback.props ```.
+ Start a client using this config.
- ``` pipewire -c filter-chain.conf ```
+ Update links manually to make ``` pw-play ``` be routed to the effect,
then to the ALSA sink node.
- This can be done using Helvum with its GUI.
- Otherwise, ``` pw-dot ``` or ``` pw-link –links ``` to get an overview then ``` pw-link <output-port> <input-port> ``` to create a new
link.
== WirePlumber
<wireplumber>
===  WirePlumber — session manager concept
- #strong[PipeWire] handles the processing of the media graph.
...
the #strong[session & policy manager].
...
- #strong[WirePlumber] implements a modular approach: it provides a
...
#strong[Lua scripts].
- Lua scripts will #strong[monitor] the PipeWire graph, and
#strong[react to events]. Configuration files also defines options
passed to scripts, including arrays matching object descriptions to
behavior for such objects.
- The default configuration is called ``` wireplumber.conf ```, see ``` /usr/share/wireplumber/wireplumber.conf ```. Default scripts are located alongside, in ``` scripts/ ```.
===  WirePlumber — configuration (2)
- Let’s look at the configuration of the ALSA monitor in ``` scripts/monitors/alsa.lua ```:
```lua
...
- Another task of the session & policy manager is #strong[permission
management].
...
- Client joining ``` pipewire-0 ``` must be given permissions by the session manager, i.e. WirePlumber. Propery ``` pipewire.access ``` is ``` "default" ```.
...
===  C API — SPA
- A building block is worth mentioning, #strong[Simple Plugin API]
...
===  C API — proxies
- Think as #strong[proxies] as file descriptors for PipeWire objects.
They are local references to global PipeWire objects.
- The equivalent on the daemon side are called #strong[resources].
- A client starts with two proxies:
+ One pointing to the ``` Core ``` object.
+ Another one to the global ``` Client ``` object that represents
itself.
===  C API — registry
- The PipeWire daemon handles a list of objects. Those are known as #strong[global] objects and are represented by ``` pw_global ```
structures.
...
- #strong[Format negociation:] the client can expose multiple
supported formats and negociation will occur when changing from
idle to running.
- #strong[Format conversion:] sample type, planar/interleaved,
