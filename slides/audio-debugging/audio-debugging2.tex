...
=== {Troubleshooting: no sound}
  ...
  \begin{itemize}
  \item Unmute ``` Master ``` and the relevant controls
...
=== [fragile]{Troubleshooting: write error}
  \begin{block}{}
...
=== [fragile]{Troubleshooting: over/underruns}
  \begin{block}{}
    \fontsize{10}{10}\selectfont
    \begin{minted}{console}
# aplay test.wav 
...
=== {Troubleshooting: going further}
  \begin{itemize}
  \item Use ``` speaker-test ``` to generate audio and play tones.
...
=== {Troubleshooting: going further}
  ...
  \item Have a look at the CPU DAI driver and its callback. In
    particular: ``` .set_clkdiv ``` and ``` .set_sysclk ``` to
    understand how the various clock dividers are setup.
    ``` .hw_params ``` or ``` .set_dai_fmt ``` may do some muxing
  \item Have a look at the codec driver callbacks, ``` .set_sysclk ```
    as the ``` clk_id ``` parameter is codec specific.
  \item Remember using a codec as a clock consumer is an uncommon
    configuration and is probably untested.
  \item When in doubt, use ``` devmem ``` or ``` i2cget ```
  \end{itemize}
