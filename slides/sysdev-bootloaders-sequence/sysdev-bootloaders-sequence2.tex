...
=== {UEFI booting}
  ...
  \item Special partition, formatted with the {\em FAT} filesystem
    \begin{itemize}
    \item MBR: identified by type `0xEF`
    \item GPT: identified with a specific {\em globally unique identifier}
    \end{itemize}
  \item File `/efi/boot/bootx32.efi`, `/efi/boot/bootx64.efi`
...
[
    ...
    \item `syslinux`: booting from FAT filesystem
    \item `pxelinux`: booting from the network
    \item `isolinux`: booting from CD-ROM
    \item `extlinux`: booting from numerous filesystem types
