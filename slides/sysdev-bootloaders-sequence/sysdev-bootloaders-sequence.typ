...
- Stage 2 is typically stored outside of any filesystem, at a fixed
  offset \$arrow.r\$\_simpler to load by stage 1
...
- Loads EFI binaries from the #emph[EFI System Partition]

  - Generally a bootloader

  - Can also be directly the Linux kernel, with an #emph[EFI Boot Stub]

- Special partition, formatted with the #emph[FAT] filesystem

  - MBR: identified by type `0xEF`

  - GPT: identified with a specific #emph[globally unique identifier]
...
#align(center, [#image("uefi-sequence.pdf", width: 70%)])

#align(center, [#image("uefi-storage.pdf", width: 90%)])
...
- #emph[Open standard that operating systems can use to discover and
  configure computer hardware components, to perform power management,
  to perform auto configuration, and to perform status monitoring]

- #emph[Tables] with descriptions of the hardware that cannot be
...
- ARM is also pushing for the adoption of UEFI and ACPI as part of its
  #emph[ARM System Ready] certification
...
- When an embedded platform uses UEFI \$arrow.r\$\_its booting process is
  similar to an #emph[x86] platform

== Booting on embedded platforms
<booting-on-embedded-platforms>
===  Booting on embedded platforms: ROM code

- Most embedded processors include a #strong[ROM code] that implements
...
- #emph[Generally] runs with the external RAM not initialized, so it can
...
#align(center, [#image("stm32mp1-rom-code.png", height: 850%)])
...
#align(center, [#image("am335x-rom-code.png", height: 850%)])
...
#align(center, [#image("two-step-boot-process.pdf", height: 850%)])

===  ROM code recovery mechanism

- Most ROM code also provide some sort of #emph[recovery] mechanism,
...
#align(center, [#image("stm32mp1-rom-code-recovery.pdf", width: 80%)])
...
- #emph[Grand Unified Bootloader], from the GNU project
...
#align(center, [#image("grub2.png", width: 80%)])
...
#align(center, [#image("syslinux.png", width: 80%)])
...
- Configured using files stored in the #emph[EFI System Partition]

- Part of the #emph[systemd] project, even though obviously distinct
  from #emph[systemd] itself
...
#align(center, [#image("systemd-boot.png", width: 80%)])
...
- Initially developed as an alternative to U-Boot to address some U-Boot
  ...
  - #emph[kconfig] for the configuration like the Linux kernel

  - well-defined #emph[device model] internally
...
- Modern SoCs have advanced security mechanisms that require running
  some sort of #emph[trusted firmware]

- This firmware is loaded by the bootloader, or part of the boot chain
  itself

- This #emph[trusted firmware] #strong[stays resident] after control has
...
- Modern ARMv7 and ARMv8 processors have
  ...
  - 4 privilege levels (#emph[Exception Levels])
  ...
  - 2 #emph[worlds]
    ...
    - Secure world, to run a separate, isolated, secure operating system
      and applications. Also called #emph[TrustZone] by ARM.
...
#align(center, [#image("arm-exception-levels.png", height: 750%)])
...
#align(center, [#image("arm-interfaces.pdf", width: 80%)])

])

===  TF-A

- #emph[Trusted Firmware-A (TF-A) provides a reference implementation of
  secure world software for Armv7-A and Armv8-A, including a Secure
  Monitor executing at Exception Level 3 (EL3)]

- Formerly known as #emph[ATF], for ARM Trusted Firmware
...
- A trusted operating system can run in the #emph[secure world], also
  called #emph[Trusted Execution Environment] or #emph[TEE]

- Hardware partitioning between #emph[secure world] and #emph[normal
  world]

  - Some hardware resources only available in the #emph[secure world],
    by the trusted OS
...
- Most common open-source implementation: #emph[OP-TEE]
...
Largely inspired from #emph[Ahmad Fatoum] presentation #emph[From Reset
Vector to Kernel],
...
- RISC-V has defined SBI, #emph[Supervisor Binary Interface]
...
also possible to use the U-Boot SPL \$arrow.r\$\_U-Boot boot flow on i.MX6.
...
See
#link("https://u-boot.readthedocs.io/en/latest/board/ti/am62x_sk.html")[https://u-boot.readthedocs.io/en/latest/board/ti/am62x_sk.html]
for details.
