...
=== Basic applications
  ...
  \item In order to work, a Linux system needs at least a few
    applications
  \item An `init` application, which is the first user space
    ...
    \item The kernel tries to run the command specified by the
          `init=` command line parameter if available.
    \item Otherwise, it tries to run `/sbin/init`, `/etc/init`,
      `/bin/init` and `/bin/sh`.
    \item In the case of an initramfs, it will only look for
      `/init`. Another path can be supplied by the `rdinit=`
      kernel argument.
  ...
  \item Basic UNIX executables, for use in system scripts or in
        interactive shells: `mv`, `cp`, `mkdir`, `cat`,
