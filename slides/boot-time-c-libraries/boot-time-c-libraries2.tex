...
=== {glibc vs uclibc-ng vs musl - small static executables}
  Let's compile and strip a `hello.c` program {\bf statically} and compare the size
  \begin{itemize}
...
=== {glibc vs uclibc vs musl - more realistic example}
  ...
  Notes:
  \begin{itemize}
    \item Tests run with `gcc 10.0.2` toolchains for `armv7-eabihf`
    \item BusyBox is automatically compiled with `-Os` and stripped.
