...
=== mount / umount
  \begin{itemize}
  \item `mount` allows to mount filesystems
    \begin{itemize}
    \item `mount -t type device mountpoint`
    \item `type` is the type of filesystem (optional for
	non-virtual filesystems)
    \item `device` is the storage device, or network location to
      mount
    \item `mountpoint` is the directory where files of the
      storage device or network location will be accessible
    \item `mount` with no arguments shows the currently mounted
      filesystems
    \end{itemize}
  \item `umount` allows to unmount filesystems
    \begin{itemize}
...
=== Root filesystem
  ...
  \item As `mount` and `umount` are programs, they are files
    inside a filesystem.
    \begin{itemize}
...
=== Mounting rootfs from storage devices
  ...
  \item Partitions of a hard disk or USB key
    \begin{itemize}
    \item `root=/dev/sdXY`, where `X` is a letter indicating
      the device, and `Y` a number indicating the partition
    \item `/dev/sdb2` is the second partition of the second disk
      drive (either USB key or ATA hard drive)
    \end{itemize}
  \item Partitions of an SD card
    \begin{itemize}
    \item `root=/dev/mmcblkXpY`, where `X` is a number
      indicating the device and `Y` a number indicating the
      partition
    \item `/dev/mmcblk0p2` is the second partition of the first
      device
    \end{itemize}
  \item Partitions of flash storage
    \begin{itemize}
    \item `root=/dev/mtdblockX`, where `X` is the partition number
    \item `/dev/mtdblock3` is the fourth enumerated flash partition in the system
...
=== Mounting rootfs over the network (2)
  ...
  \item Add the exported directory to your `/etc/exports` file:\\
    ...
    \begin{itemize}
    \item `192.168.1.111` is the client IP address
    \item `rw,no_root_squash,no_subtree_check` are the NFS server
...
=== Mounting rootfs over the network (3)
  ...
  \item The kernel must be booted with the following parameters:
    \begin{itemize}
    \item `root=/dev/nfs` (we want rootfs over NFS)
    \item `ip=192.168.1.111` (target IP address)
    \item `nfsroot=192.168.1.110:/home/tux/rootfs/` (NFS server details)
    \item You may need to add "`,nfsvers=3,tcp`" to the
      `nfsroot` setting, as an NFS version 2 client and UDP may
