...
=== [fragile]{Cross-compiling with hand-written Makefiles}
  \begin{itemize}
  \item There is no general rule, as each project has a different
    set of Makefiles, that use a different set of variables
  \item Though it is common to use `make` standard variables:
    `CC` (C compiler path), `CXX` (C++ compiler path),
    `LD` (linker path), `CFLAGS` (C compiler flags),
    `CXXFLAGS` (C++ compiler flags), `LDFLAGS` (linker
    flags)
  \item `DESTDIR` for installation destination, sometimes
    `PREFIX` for execution location
...
=== [fragile]{Example: {\em uftp} native compilation}
...
=== [fragile]{Example: {\em uftp} cross-compilation}
...
\end{verbatim}
  ...
  \item This is an optional dependency that can be disabled using the
    special `make` variable `NO_ENCRYPTION`
  \end{itemize}
...
=== [fragile]{Example: {\em OpenSSL} cross-compilation}

  
#table(columns: (50\%, 50\%), stroke: none,
[
  OpenSSL has a hand-written `Configure` shell script that needs
...
=== [fragile]{Example: {\em uftp} with {\em OpenSSL} support}
...
\end{verbatim}
  ...
  {\small It cannot find the header, let's add `CFLAGS` pointing
...
\end{verbatim}
  ...
  {\small Compilation of object files work, but link fails as the
    linker cannot find the OpenSSL library. Let's add `LDFLAGS`
...
\end{verbatim}
  ...
  {\small We can indeed see that `uftp` is linked against the
...
=== {automake / autoconf}
  ...
  \item Files written by the developer
    \begin{itemize}
    \item `configure.in` describes the configuration options and
      the checks done at configure time
    \item `Makefile.am` describes how the software should be
      built
    \end{itemize}
  \item The `configure` script and the `Makefile.in` files
    are generated by `autoconf` and `automake` respectively.
    \begin{itemize}
    ...
    \end{itemize}
  \item The `Makefile` files are generated at configure time, before
    compiling
    \begin{itemize}
...
=== {autotools usage: four steps}
  \begin{enumerate}
  \item Only if needed: generate `configure` and
    `Makefile.in`. Either using {\em autoreconf} tool, or
    sometimes an `autogen.sh` script is provided by the package
  \item {\bf Configuration:} `./configure`
    \begin{itemize}
    \item `./configure --help` is very useful
    \item `--prefix`: execution location
    \item `--host`: target machine when cross-compiling, if not
      provided, auto-detected. Also used as the cross-compiler prefix.
    \item Often `--enable-<foo>`, `--disable-<foo>`,
      `--with-<foo>`, `--without-<foo>` for optional
      features.
    \item `CC`, `CXX`, `CFLAGS`, `CXXFLAGS`,
      `LDFLAGS` and many more variables
    \end{itemize}
  \item {\bf Build:} `make`
  \item {\bf Installation:} `make install`
    \begin{itemize}
    \item `DESTDIR` variable for {\em diverted installation}
    \end{itemize}
  \end{enumerate}


=== [fragile]{Example: can-utils native compilation}
...
\end{verbatim}
  ...
  {\small No `configure` and
    `GNUmakefile.in`, {\em autoreconf needed}.}
...
\end{verbatim}
  ...
  {\small We now have the `GNUmakefile`, we can build and install.}
...
=== [fragile]{Example: {\em can-utils} cross-compilation}
  \begin{block}{}
    {\scriptsize
\begin{verbatim}
$ export PATH=/xtools/gcc-arm-10.3-2021.07-x86_64-arm-none-linux-gnueabihf/bin:$PATH
...
=== {CMake}
  ...
  \item Needs `cmake` installed on your machine
  \item Based on:
    \begin{itemize}
    \item `CMakeLists.txt` files that describe what the
      dependencies are and what to build and install
    \item `cmake`, a tool that processes `CMakeLists.txt` to
...
=== [fragile]{Example: {\em cJSON} native compilation}
...
=== Example: {\em cJSON cross-compilation}
...
=== {CMake {\em toolchain file}}
  ...
  \item They can be grouped into a {\em toolchain file}, which defines
    {\em CMake} variables
  \item Can then be used with `cmake
...
=== {Meson}
  ...
  \item Processes `meson.build` + `meson_options.txt` and
    generates {\em Ninja} files
  \item {\em Ninja} is an alternative to `make`, with much
    shorter build times
  \item Needs `meson` and `ninja` installed on your machine
...
=== [fragile]{Example: {\em ipcalc} native compilation}
...
[
    ...
    \item `--cross-file` option of {\em Meson}
    \end{itemize}
    \column{0.5\textwidth}
...
=== [fragile]{Example: {\em ipcalc} cross-compilation}
    \begin{block}{}
      {\footnotesize
\begin{verbatim}
$ cat cross-file.txt
...
=== {Distinction between {\em prefix} and {\em DESTDIR}}
...
[
    ...
    \item `prefix`: where the software will be executed from on the
      target
    \item `DESTDIR`: where the software is installed by the build
      system installation procedure. Allows to install in a different
      place than `prefix`, when creating a root filesystem for a
      different machine.
    \end{itemize}
    \column{0.5\textwidth}
...
=== {pkg-config}
  \begin{itemize}
  \item `pkg-config` is a tool that allows to query a small
  ...
  \item The database is made of `.pc` files, installed by default in
    `<prefix>/lib/pkgconfig/`.
  \item `pkg-config` is often used by {\em autotools}, {\em
      CMake}, {\em Meson} to find libraries
  \item By default, `pkg-config` looks in
    `/usr/lib/pkgconfig` for the `*.pc` files, and assumes
    that the paths in these files are correct.
  \item `PKG_CONFIG_LIBDIR` allows to set another location for the
    `*.pc` files.
  \item `PKG_CONFIG_SYSROOT_DIR` allows to prepend a directory to the
    paths mentioned in the `.pc` files and appearing in the
    `pkg-config` output.
  \end{itemize}


=== [fragile]{pkg-config example for native compilation}
  \begin{block}{}
    {\scriptsize
\begin{verbatim}
$ pkg-config --list-all openssl                        OpenSSL - Secure Sockets Layer and cryptography libraries and tools zlib                           zlib - zlib compression library blkid                          blkid - Block device id library cairo-script                   cairo-script - script surface backend for cairo graphics library cairo-pdf                      cairo-pdf - PDF surface backend for cairo graphics library xcb-xinput                     XCB XInput - XCB XInput Extension (EXPERIMENTAL)
...
=== [fragile]{pkg-config example for cross-compilation}
  \begin{block}{Use `PKG_CONFIG_LIBDIR`}
    {\scriptsize
\begin{verbatim}
$ export PKG_CONFIG_LIBDIR=/tmp/staging/usr/lib/pkgconfig
$ pkg-config --list-all openssl                        OpenSSL - Secure Sockets Layer and cryptography libraries and tools libssl                         OpenSSL-libssl - Secure Sockets Layer and cryptography libraries libcrypto                      OpenSSL-libcrypto - OpenSSL cryptography library
$ pkg-config --cflags --libs openssl
...
\end{verbatim}
  ...
  \begin{block}{Use `PKG_CONFIG_SYSROOT_DIR`}
    {\scriptsize
\begin{verbatim}
...
#setuplabframe([Cross-compiling applications and libraries],[
  ...
  \item This includes compiling {\em alsa-utils} package,
    and using its `speaker-test` program to test that
