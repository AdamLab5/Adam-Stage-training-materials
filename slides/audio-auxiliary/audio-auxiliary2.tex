...
=== [fragile]{simple-amplifier - example 1}
...
=== [fragile]{simple-amplifier - example 2}
  \begin{block}{}
    \fontsize{7}{6}\selectfont
    \begin{minted}{c}
...
=== {Input Muxing}
  ...
  \item If controlled using a gpio, the `simple-mux` driver is
    available.
...
=== [fragile]{`simple-mux` example}
  ...
  \end{block}
  \begin{itemize}
  \item This exposes routes between `Mic Mux IN1` and `Mic
    Mux IN2` to `Mic Mux OUT`.
  \item This route is controlled by `gpio5 5`.
  \item A control named `Mic Mux Muxer` will be exposed to
    userspace.
  \end{itemize}
