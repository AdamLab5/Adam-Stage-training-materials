...
=== [fragile]{Configuration system}
  \begin{itemize}
  \item Uses, almost unchanged, the {\em kconfig} code from the
    kernel, in `support/kconfig` (variable `CONFIG`)
  \item {\em kconfig} tools are built in
    `\$(BUILD_DIR)/buildroot-config/`
  \item The main `Config.in` file, passed to *config, is at the
...
=== [fragile]{Where is {\tt \$(PACKAGES)} filled?}

\begin{block}{Part of `package/pkg-generic.mk`}
\begin{minted}[fontsize=\tiny]{make}
...
\item `package/pkg-generic.mk` is really the core of the package
  infrastructure
\end{itemize}



=== {Diving into {\tt pkg-generic.mk}}

\begin{itemize}
\item The `package/pkg-generic.mk` file is divided in two main
  parts:
  \begin{enumerate}
  \item Definition of the actions done in each step of a package build
    process. Done through {\em stamp file targets}.
  \item Definition of the `inner-generic-package`,
    `generic-package` and `host-generic-package` macros,
...
=== [fragile]{Definition of the actions: code}
...
=== [fragile]{Action example 1: download}
...
\item In all {\em stamp file targets}, `PKG` is the upper case
  name of the package. So when used for BusyBox,
  `\$(\$(PKG)_SOURCE)` is the value of `BUSYBOX_SOURCE`.
\item {\em Hooks}: make macros called before and after each step.
\item `<pkg>_ALL_DOWNLOADS` lists all the files to be downloaded,
  which includes the ones listed in `<pkg>_SOURCE`,
  `<pkg>_EXTRA_DOWNLOADS` and `<pkg>_PATCH`.
\end{itemize}



=== [fragile]{Action example 2: build}
...
\item `step_start` and `step_end` are part of
...
=== [fragile]{The {\tt generic-package} macro}
...
\item In `package/libzlib/libzlib.mk`:
\begin{block}{}
\begin{minted}[fontsize=\tiny]{make}
LIBZLIB_... = ...
...
=== [fragile]{`inner-generic-package`: defining variables}
...
\end{block}
 ...
 #colbreak()
\begin{block}{Expanded for `host-libzlib`}
\begin{minted}[fontsize=\tiny]{make}
...
=== [fragile]{`inner-generic-package`: dependencies}
...
\begin{itemize}
\item Adding the `skeleton` and `toolchain` dependencies to
  target packages. Except for some specific packages (e.g. C library).
\end{itemize}


=== [fragile]{`inner-generic-package`: stamp files}
...
\begin{itemize}
\item Pass variables to the stamp file targets, especially `PKG`
\end{itemize}



=== [fragile]{`inner-generic-package`: sequencing}
...
=== [fragile]{Preparation work: prepare, dependencies}
...
\$\$(\$(2)_TARGET_SOURCE): | dependencies
  ...
  \item All packages have two targets in their dependencies:
    \begin{itemize}
    \item `prepare`: generates a kconfig-related `auto.conf`
      file
    \item `dependencies`: triggers the check of Buildroot system
...
#columns(gutter: 8pt)[
  ...
  \item One can force rebuilding a package from its configure, build
    or install step using `make <pkg>-reconfigure`,
    `make <pkg>-rebuild` or `make <pkg>-reinstall`
  \end{itemize}
     #colbreak()
  \begin{block}{}
\begin{minted}[fontsize=\tiny]{make}
...
=== {Specialized package infrastructures}
  \begin{itemize}
  \item The `generic-package` infrastructure is fine for packages
...
=== [fragile]{CMake package example: {\tt flann}}
...
=== [fragile]{CMake package infrastructure (1/2)}
...
=== [fragile]{CMake package infrastructure (2/2)}
...
=== [fragile]{Autoreconf in `pkg-autotools.mk`}
  \begin{itemize}
  \item Package infrastructures can also add additional capabilities
    controlled by variables in packages
  \item For example, with the `autotools-package` infra, one can
    do `FOOBAR_AUTORECONF = YES` in a package to trigger an {\em
      autoreconf} before the {\em configure} script is executed
  \item Implementation in `pkg-autotools.mk`
    \begin{block}{}
      \begin{minted}[fontsize=\tiny]{make}
...
=== {Toolchain support}
  \begin{itemize}
  \item One {\em virtual package}, `toolchain`, with two
    implementations in the form of two packages:
    `toolchain-buildroot` and `toolchain-external`
  \item `toolchain-buildroot` implements the {\bf internal
  ...
  \item `toolchain-external` implements the {\bf external
...
#columns(gutter: 8pt)[
  ...
  \item Build starts with utility host tools and libraries needed for
    gcc (`host-m4`, `host-mpc`, `host-mpfr`,
    `host-gmp`). Installed in
    `\$(HOST_DIR)/{bin,include,lib`}
  \item Build goes on with the cross binutils, `host-binutils`,
    installed in `\$(HOST_DIR)/bin`
  \item Then the first stage compiler, `host-gcc-initial`
  \item We need the `linux-headers`, installed in
    `\$(STAGING_DIR)/usr/include`
  \item We build the C library, `uclibc` in this example. Installed
    in `\$(STAGING_DIR)/lib`, `\$(STAGING_DIR)/usr/include`
    and of course `\$(TARGET_DIR)/lib`
  \item We build the final compiler `host-gcc-final`, installed
    in `\$(HOST_DIR)/bin`
  \end{itemize}
   #colbreak()
...
#columns(gutter: 8pt)[
    \begin{itemize}
    \item `toolchain-external-package` infrastructure, implementing
      the common logic for all external toolchains
      \begin{itemize}
      \item Implemented in `toolchain/toolchain-external/pkg-toolchain-external.mk`
      \end{itemize}
    \item Packages in `toolchain/toolchain-external/` are using
      this infrastructure
      \begin{itemize}
      \item E.g. `toolchain-external-arm-aarch64`,
        `toolchain-external-bootlin`
      \end{itemize}
    \item `toolchain-external` is a virtual package itself
...
=== [fragile]{External toolchain example}
...
=== {{\tt toolchain-external-package} logic}
  \begin{enumerate}
  \item Extract the toolchain to `\$(HOST_DIR)/opt/ext-toolchain`
  ...
  \item Copy the toolchain {\em sysroot} (C library and headers,
    kernel headers) to `\$(STAGING_DIR)/usr/{include,lib`}
  \item Copy the toolchain libraries to `\$(TARGET_DIR)/usr/lib`
  \item Create symbolic links or wrappers for the compiler, linker,
    debugger, etc from `\$(HOST_DIR)/bin/<tuple>-<tool>` to
  ...
  \item A wrapper program is used for certain tools (gcc, ld, g++,
    ...
    used, especially `--sysroot=\$(STAGING_DIR)` and
...
=== {Root filesystem image generation}
  \begin{itemize}
  \item Once all the targets in `\$(PACKAGES)` have been built,
    it's time to create the root filesystem images
  \item First, the `target-finalize` target does some cleanup of
...
=== [fragile]{{\tt fs/common.mk}, {\tt rootfs} infrastructure 1}
  \begin{block}{}
\begin{minted}[fontsize=\tiny]{make}
define inner-rootfs
...
=== [fragile]{{\tt fs/common.mk}, {\tt rootfs} infrastructure 2}
  \begin{block}{}
\begin{minted}[fontsize=\tiny]{make}
