...
=== Toolchain definition (1)
  ...
  \item This toolchain runs on your workstation and generates binary code for
...
=== {Architecture tuple and toolchain prefix}
  ...
  \item Examples: `arm-linux-gnueabihf`,
    `mips64el-linux-gnu`, `arm-vendor-none-eabihf`
  \item These tuples are 3 or 4 parts:
    \begin{enumerate}
    \item The architecture name: `arm`, `riscv`,
      `mips64el`, etc.
    \item Optionally, a vendor name, which is a free-form string
    \item An operating system name, or `none` when not targeting
      an operating system
  ...
  \item This tuple is used to:
    ...
    \item as a prefix of cross-compilation tools, to differentiate
      ...
      \begin{itemize}
      \item `gcc` $\rightarrow$ native compiler
      \item `arm-linux-gnueabihf-gcc` $\rightarrow$ cross-compiler
      \end{itemize}
    \end{itemize}
  \end{itemize}
...
=== Binutils
  ...
  \item {\bf Binutils} is a set of tools to generate and manipulate
    ...
    \begin{itemize}
    \item `as`, the assembler, that generates binary code from
      assembler source code
    \item `ld`, the linker
    \item `ar`, `ranlib`, to generate `.a` archives
     (static libraries)
    \item `objdump`, `readelf`, `size`, `nm`,
      `strings`, to inspect binaries. Very useful analysis tools!
    \item `objcopy`, to modify binaries
    \item `strip`, to strip parts of binaries that are just needed
...
[
    ...
    \item Available in `<linux/...>` and `<asm/...>` and a few
      other directories corresponding to the ones visible in
      #kdir("include/uapi") and in `arch/<arch>/include/uapi` in the kernel sources
...
=== Kernel headers (2)
  \begin{itemize}
  \item System call numbers, in `<asm/unistd.h>`
\begin{verbatim}
#define __NR_exit         1
#define __NR_fork         2
#define __NR_read         3
\end{verbatim}
  \item Constant definitions, here in `<asm-generic/fcntl.h>`,
    included from `<asm/fcntl.h>`, included from
    `<linux/fcntl.h>`
\begin{verbatim}
#define O_RDWR 00000002
\end{verbatim}
\item Data structures, here in `<asm/stat.h>` (used by the
`stat` command)
\begin{verbatim}
struct stat {
...
=== Kernel headers (3)
  ...
  it's usually fine to keep an old toolchain as long as it works fine
  for your project.
...
[
    ...
    \item By default, quite big for small embedded systems.
      On armv7hf, version 2.31: `libc`: 1.5 MB, `libm`: 432
...
=== uClibc-ng
  ...
  \item Lightweight C standard library for small embedded systems
    ...
    \item Size on armv7hf, version 1.0.34:
      `libc`: 712 KB, source: \url{https://toolchains.bootlin.com}
    \end{itemize}
...
[
      ...
      \item Size on armv7hf, version 1.2.0:
        `libc`: 748 KB, source: \url{https://toolchains.bootlin.com}
      \end{itemize}
...
=== {Linux vs. bare-metal toolchain}
  ...
  \item A {\bf Linux toolchain}
    ...
    \item is identified by the `linux` OS identifier in the
      toolchain tuple: `arm-linux`,
  ...
  \item A {\bf bare metal toolchain}
    ...
    \item is identified by the `none` OS identifier in the
      toolchain tuple: `arm-none-eabi`, `arm-none-none-eabi`
      (vendor is `none`, OS is `none`)
    \end{itemize}
  \end{itemize}
...
=== {An alternate compiler suite: LLVM}
  \begin{itemize}
  \item Most Embedded Linux projects use toolchains based on the GNU
