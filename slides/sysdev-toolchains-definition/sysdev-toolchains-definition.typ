...
- Therefore, #strong[cross-compiling toolchains] are generally used.
...
- Many UNIX/Linux build mechanisms rely on #emph[architecture tuple]
...
- This tuple is used to:
  ...
  - as a prefix of cross-compilation tools, to differentiate them from
    ...
    - `gcc` \$arrow.r\$\_native compiler

    - `arm-linux-gnueabihf-gcc` \$arrow.r\$\_cross-compiler

===  Components of gcc toolchains

#align(center, [#image("components.pdf", width: 80%)])

===  Binutils

- #strong[Binutils] is a set of tools to generate and manipulate
...
===  Kernel headers (3) The kernel to user space interface is
#strong[backward compatible]

- Kernel developers are doing their best to #strong[never] break
...
- Several C standard libraries are available: #emph[glibc],
  #emph[uClibc], #emph[musl], #emph[klibc], #emph[newlib]...
...
- Advice to start developing and debugging your applications with
  #emph[glibc], which is the most standard solution

- If you have size constraints, try to compile your app and then the
  entire filesystem with #emph[uClibc] or #emph[musl]

  - The size advantage of #emph[uClibc] or #emph[musl], which used to be
