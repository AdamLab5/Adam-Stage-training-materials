...
- One only interacts with the main #kfile("Makefile"), present at the
  #strong[top directory] of the kernel source tree

- Interaction takes place

  - using the `make` tool, which parses the Makefile

  - through various #strong[targets], defining which action should be
...
===  Specifying the target architecture First, specify the
architecture for the kernel to build
...
===  Choosing a compiler The compiler invoked by the kernel Makefile
...
===  Specifying ARCH and CROSS_COMPILE There are actually two ways of
defining `ARCH` and `CROSS_COMPILE`:
...
===  Initial configuration Difficult to find which kernel
configuration will work with your hardware and root filesystem. Start
with one that works!
...
- When you use an embedded build system (Buildroot, OpenEmbedded) use
  its specific commands. E.g. `make linux-menuconfig` and `make
  linux-update-defconfig` in Buildroot.

===  Built-in or module?

- The #strong[kernel image] is a #strong[single file], resulting from
...
- Some features (device drivers, filesystems, etc.) can however be
  compiled as #strong[modules]

  - These are #emph[plugins] that can be loaded/unloaded dynamically to
    add/remove features to the kernel

  - Each #strong[module is stored as a separate file in the filesystem],
...
===  Kernel option types There are different types of options, defined
in `Kconfig` files:

- `bool` options, they are either

  - #emph[true] (to include the feature in the kernel) or

  - #emph[false] (to exclude the feature from the kernel)

- `tristate` options, they are either

  - #emph[true] (to include the feature in the kernel image) or

  - #emph[module] (to include the feature as a kernel module) or

  - #emph[false] (to exclude the feature)
...
===  Kernel option dependencies Enabling a network driver requires the
network stack to be enabled, therefore configuration symbols have two
...
#table(columns: (50%, 50%), stroke: none, [ `make xconfig`
...
#table(columns: (50%, 50%), stroke: none, [ `make menuconfig`
...
===  Kernel configuration options You can switch from one tool to
another, they all load/save the same `.config` file, and show the same
set of options
...
If you edit a `.config` file by hand, it`s useful to run `make
oldconfig` afterwards, to set values to new parameters that could have
appeared because of dependency changes.

===  Undoing configuration changes A frequent problem:
...
#table(columns: (50%, 50%), stroke: none, [ `make`

- Only works from the top kernel source directory

- Should not be performed as a privileged user

- Run several #strong[j]obs in parallel. Our advice: `\$(nproc)` to
  fully load the CPU and I/Os at all times. 
  Example: `make -j20`

- To #strong[re]compile faster (7x according to some benchmarks), 
...
#align(center, [#image("kernel-mrproper.png", width: 90%)])

])

===  Kernel building overview

#align(center, [#image("kernel-building-overview.pdf", height: 80%)])
