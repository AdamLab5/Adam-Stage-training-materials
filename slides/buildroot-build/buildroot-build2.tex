...
=== {Default build organization}
  ...
  \item All the build output goes into a directory called
    `output/` within the top-level Buildroot source directory.
    \begin{itemize}
    \item `O = output`
    \end{itemize}
  \item The configuration file is stored as `.config` in the
    top-level Buildroot source directory.
    \begin{itemize}
    \item `CONFIG_DIR = \$(TOPDIR)`
    \item `TOPDIR = \$(shell pwd)`
    \end{itemize}
  \item `buildroot/`
    \begin{itemize}
    \item {\bf `.config`}
...
=== {Out of tree build: introduction}
  ...
  \item Configuration file stored inside the `\$(O)` directory, as
    opposed to inside the Buildroot sources for the in-tree build
    case.
  \item `project/`
    \begin{itemize}
    \item `buildroot/`, Buildroot sources
    \item `foo-output/`, output of a first project
      \begin{itemize}
      \item `.config`
      \end{itemize}
    \item `bar-output/`, output of a second project
      \begin{itemize}
      \item `.config`
      \end{itemize}
    \end{itemize}
...
=== [fragile]{Out of tree build: using}
  \begin{itemize}
  \item To start an out of tree build, two solutions:
    \begin{itemize}
    \item From the Buildroot source tree, simplify specify a `O=`
      variable:
      \begin{block}{}
\begin{verbatim}
...
\end{verbatim}
      \end{block}
    \item From an empty output directory, specify `O=` and the
...
\end{verbatim}
  ...
  \item Once one out of tree operation has been done
    (`menuconfig`, loading a defconfig, etc.), Buildroot creates
    a small wrapper `Makefile` in the output directory.
  \item This wrapper `Makefile` then avoids the need to pass
    `O=` and the path to the Buildroot source tree.
  \end{itemize}
...
\end{block}
  ...
  \item Start the build (passing `O=` and `-C` no longer needed thanks to the wrapper):\\
{\tiny
\begin{block}{}
\begin{verbatim}
$ make
...
=== {Full config file vs. {\em defconfig}}
  \begin{itemize}
  \item The `.config` file is a {\em full} config file: it
    contains the value for all options (except those having unmet
    dependencies)
  \item The default `.config`, without any customization, has
...
\end{block}
  ...
  \item If then you also enable the `stress` package, the {\em
...
=== {Using and creating a {\em defconfig}}
  \begin{itemize}
  \item To use a {\em defconfig}, copying it to `.config` is not
    sufficient as all the missing (default) options need to be
    expanded.
  \item Buildroot allows to load {\em defconfig} stored in the
    `configs/` directory, by doing:
    `make <foo>_defconfig`
    \begin{itemize}
    \item It overwrites the current `.config`, if any
    \end{itemize}
  \item To create a {\em defconfig}, run:\\
    `make savedefconfig`
    \begin{itemize}
    \item Saved in the file pointed by the `BR2_DEFCONFIG`
      configuration option
    \item By default, points to `defconfig` in the current
    ...
    \item Move it to `configs/` to make it easily loadable with
      `make <foo>_defconfig`.
    \end{itemize}
  \end{itemize}


=== [fragile]{Existing {\em defconfigs}}
  ...
  \item Buildroot comes with a number of existing {\em defconfigs} for
    ...
    \end{itemize}
  \item List them using `make list-defconfigs`
...
\item Additional instructions often available in
  `board/<boardname>`, e.g.: `board/qemu/arm-vexpress/readme.txt`.
\item Your own {\em defconfigs} can obviously be more featureful
  \end{itemize}


=== [fragile]{Assembling a {\em defconfig} (1/2)}
...
\begin{verbatim}
BR2_arm=y BR2_TOOLCHAIN_BUILDROOT_WCHAR=y BR2_GCC_VERSION_7_X=y
\end{verbatim}
   \end{block}
}
...
\begin{verbatim}
BR2_mipsel=y BR2_TOOLCHAIN_EXTERNAL=y BR2_TOOLCHAIN_EXTERNAL_CODESOURCERY_MIPS=y
\end{verbatim}
    \end{block}
}
...
\begin{verbatim}
BR2_PACKAGE_STRESS=y BR2_PACKAGE_MTD=y BR2_PACKAGE_LIBCONFIG=y
\end{verbatim}
   \end{block}
}
...
=== [fragile]{Assembling a {\em defconfig} (2/2)}
...
\end{verbatim}
  ...
  \item Verbose build
    ...
    \item To get a fully verbose build, pass `V=1`:
      \begin{block}{}
\begin{verbatim}
$ make V=1
\end{verbatim}
      \end{block}
    \item Passing `V=1` also applies to packages, like the Linux
      kernel, busybox...
    \end{itemize}
  \end{itemize}
