...
=== Yocto: principle
  ...
  \item The \href{https://docs.yoctoproject.org/_images/yp-flow-diagram.svg}
  \end{itemize}
...
=== Lexicon: ```bitbake```
  ...
  ```bitbake``` program
  \begin{itemize}
    \item ```bitbake``` is a task scheduler, like ```make```
    \item ```bitbake``` parses text files
      %called {\em metadata}
...
=== Lexicon: metadata and layers
  \begin{itemize}
    \item The input to ```bitbake``` is collectively called {\em metadata}
...
=== 
  ...
  \begin{itemize}
    ...
    \item Each release has a codename such as ```kirkstone``` or ```scarthgap```,
