...
= Optimizing init scripts and system startup ===  Methodology There
are multiple ways to reduce the time spent in init scripts before
...
===  Measuring - systemd If you are using `systemd` as your `init`
program, you can use `systemd-analyze`. See
...
#columns(gutter: 8pt)[ This command prints an SVG graphic detailing
...
===  Init optimizations Goal to start your application as soon as
possible after all the dependencies are started:
...
===  Reduce forking (2) Pipes and back-quotes are also implemented by
`fork`/`exec`. You can reduce their usage in scripts. Example:
...
===  Quick splashscreen display (1) Often the first sign of life that
you are showing!
...
===  Animated splashscreen Still slow to read and write entire
screens. Just draw useful pixels and even create an animation!
