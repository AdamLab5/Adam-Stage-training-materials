...
- Each #emph[external] directory must contain:
...
- File giving metadata about the #emph[external tree]
...
- Can include custom #emph[make] logic
...
- Internally, each package is implemented through a number of
  package-specific #emph[make targets]
...
- Doing a #strong[full rebuild] is achieved using:
...
- Buildroot #strong[does not try to be smart]
...
- When a package has been built by Buildroot, Buildroot keeps a
  #strong[hidden file] telling that the package has been built.
...
- When `make` is invoked, Buildroot #strong[will always]:
...
- The #emph[per-package] option ensures that each package uses its own
  ...
  packages can be built in parallel with no interference
