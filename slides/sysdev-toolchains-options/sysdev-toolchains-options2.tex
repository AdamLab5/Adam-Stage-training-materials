...
=== {Floating point support}
  ...
  \item All ARMv7-A (32-bit) and ARMv8-A (64-bit) processors have a
    floating point unit
  \item RISC-V cores with the `F` extension have a floating point
    unit
...
=== CPU optimization flags
  ...
  \item gcc offers further options:
    \begin{itemize}
    \item `-march` allows to select a specific target instruction set
    \item `-mtune` allows to optimize code for a specific CPU
    \item For example: `-march=armv7 -mtune=cortex-a8`
    \item `-mcpu=cortex-a8` can be used instead to allow gcc to infer the target
      instruction set (`-march=armv7`) and cpu optimizations (`-mtune=cortex-a8`)
  ...
  \item At the GNU toolchain compilation time, values can be chosen. They are used:
    ...
    \item As the default values for the cross-compiling tools, when no
      other `-march`, `-mtune`, `-mcpu` options are
      passed
