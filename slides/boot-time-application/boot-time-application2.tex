...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item For example, easy to find file open attempts (`open()`),
		      file access (`read()`, `write()`), and
		      memory allocations (`mmap2()`). Can be done
		...
		\item Find unnecessary work done in applications
		      ...
		      do not exist.
	\end{itemize}
	\item Limitation: you can't trace the `init` process!
\end{itemize}


#include "../../common/strace.tex"

#include "../../common/ltrace.tex"

#include "../../common/valgrind.tex"
...
\begin{itemize}
	...
	\item User space tool: `perf`. It is part of the kernel
...
\end{verbatim}
	\end{block}
        \item Note: advice to run `perf` on a filesystem built with
              glibc. Didn't manage to compile `perf` on a Musl
...
\item Compile the video player with just the features needed at run
      time.
\item Trace and profile the video player with `strace`
